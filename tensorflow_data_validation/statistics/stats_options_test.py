...
INVALID_STATS_OPTIONS = [
    {
        ...
        "stats_options_kwargs": {"generators": {}},
    ...
    },
    {
        ...
        "exception_type": ValueError,
        "error_message": (
            "Categorical float features set in schema require "
...
class StatsOptionsTest(parameterized.TestCase):
    ...
    def test_valid_stats_options_json_round_trip(self):
        ...
        self.assertEqual(num_rank_histogram_buckets, options.num_rank_histogram_buckets)
        self.assertEqual(
        ...
        self.assertEqual(
            semantic_domain_stats_sample_rate, options.semantic_domain_stats_sample_rate
        )
        self.assertEqual(
            per_feature_weight_override, options._per_feature_weight_override
        )
    ...
    @parameterized.named_parameters(
        ...
        {"testcase_name": "type_name_correct", "type_name": "StatsOptions"},
        {
    ...
    def test_stats_options_from_json(
        self, type_name: Optional[str] = None, want_exception: bool = False
    ):
        if type_name:
            type_name_line = f',\n"TYPE_NAME": "{type_name}"\n'
        else:
    ...
    def test_example_weight_map(self):
        ...
        self.assertEqual(
            frozenset([]), options.example_weight_map.all_weight_features()
        )

        options = stats_options.StatsOptions(weight_feature="w")
        self.assertEqual("w", options.example_weight_map.get(types.FeaturePath(["f"])))
        self.assertEqual(
        ...
        self.assertIsNone(options.example_weight_map.get(types.FeaturePath(["f"])))
        self.assertEqual("w", options.example_weight_map.get(types.FeaturePath(["x"])))
        self.assertEqual(
            frozenset(["w"]), options.example_weight_map.all_weight_features()
        )
