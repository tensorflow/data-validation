...
from tfx_bsl.sketches import KmvSketch, MisraGriesSketch
...
class _CombinedSketch:
    ...
    def estimate(self):
        ...
        topk_unweighted_counts = [
            top_k_uniques_stats_util.FeatureValueCount(pair["values"], pair["counts"])
            for pair in topk_unweighted
        ]
        topk_weighted = self._topk_weighted.Estimate().to_pylist()
        topk_weighted_counts = [
            top_k_uniques_stats_util.FeatureValueCount(pair["values"], pair["counts"])
            for pair in topk_weighted
        ]
...
class TopKUniquesSketchStatsGenerator(stats_generator.CombinerStatsGenerator):
    ...
    def __init__(
        self,
        name: str = "TopKUniquesSketchStatsGenerator",
    ...
    def _update_combined_sketch_for_feature(
        self,
        feature_name: tfdv_types.FeaturePath,
        values: pa.Array,
        ...
        if combined_sketch is None:
            ...
            def make_mg_sketch():
                ...
                categorical = (
                    self._byte_feature_is_categorical_values.get(
                        feature_name,
    ...
    def _should_run(
        self, feature_path: tfdv_types.FeaturePath, feature_type: Optional[int]
    ) -> bool:
        ...
        if feature_type == statistics_pb2.FeatureNameStatistics.STRING:
            return (
                ...
                and self._byte_feature_is_categorical_values.get(feature_path, 0)
    ...
    def add_input(
        ...
        def update_length_counters(
            feature_type: tfdv_types.FeatureNameStatisticsType, leaf_array: pa.Array
        ):
            if np.random.random() > self._length_counter_sampling_rate:
                return
            if feature_type == statistics_pb2.FeatureNameStatistics.STRING:
                ...
                for k, v in distinct_count.items():
                    beam.metrics.Metrics.counter(
                        constants.METRICS_NAMESPACE, "binary_scalar_len_" + str(k)
                    ).inc(v)
    ...
    def merge_accumulators(
        self, accumulators: Iterable[Dict[tfdv_types.FeaturePath, _CombinedSketch]]
    ) -> Dict[tfdv_types.FeaturePath, _CombinedSketch]:
    ...
    def extract_output(
        ...
        for feature_path, combined_sketch in accumulator.items():
            ...
            if self._store_output_in_custom_stats:
                make_feature_stats_proto = top_k_uniques_stats_util.make_feature_stats_proto_topk_uniques_custom_stats

            feature_stats_proto = make_feature_stats_proto(
