...
from tensorflow_data_validation.statistics.generators import (
    natural_language_domain_inferring_stats_generator as nlsg,
)
from tensorflow_data_validation.utils import test_util


class _FakeHeuristic(nlsg.NLClassifierInterface):
    def classify(self, single_value: str) -> bool:
...
class NaturalLanguageStatsGeneratorTest(test_util.CombinerFeatureStatsGeneratorTest):
    ...
    def test_average_word_heuristic_avg_word_length_check(self):
        ...
        self.assertFalse(
            nlsg.AverageWordHeuristicNLClassifier(
                avg_word_length_min=3.5, avg_word_length_max=3.7
            ).classify(text_avg_word_length_3_8)
        )
        self.assertTrue(
            nlsg.AverageWordHeuristicNLClassifier(
                avg_word_length_min=3.7, avg_word_length_max=3.9
            ).classify(text_avg_word_length_3_8)
        )
        self.assertFalse(
            nlsg.AverageWordHeuristicNLClassifier(
                avg_word_length_min=3.9, avg_word_length_max=4.1
            ).classify(text_avg_word_length_3_8)
        )
    ...
    def test_average_word_heuristic_min_words(self):
        ...
        self.assertTrue(
            nlsg.AverageWordHeuristicNLClassifier(min_words_per_value=3).classify(
                text_5_words
            )
        )
        self.assertFalse(
            nlsg.AverageWordHeuristicNLClassifier(min_words_per_value=6).classify(
                text_5_words
            )
        )
    ...
    def test_nl_generator_bad_initialization(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "NLDomainInferringStatsGenerator expects values_threshold > 0."
        ):
