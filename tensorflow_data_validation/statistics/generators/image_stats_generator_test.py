...
class FakeImageDecoder(image_stats_generator.ImageDecoderInterface):
    ...
    def get_formats(self, value_list):
        return np.array(
            [json.loads(value)["format"] for value in value_list], dtype=object
        )

    def get_sizes(self, value_list):
        loaded_metadata = [json.loads(value) for value in value_list]
        return np.array([[meta["height"], meta["width"]] for meta in loaded_metadata])


class ImageStatsGeneratorTest(
    test_util.CombinerFeatureStatsGeneratorTest, parameterized.TestCase
):
    ...
    def test_cases_with_no_image_stats(self, batches):
        ...
        generator = image_stats_generator.ImageStatsGenerator(
            image_decoder=image_decoder, values_threshold=1, enable_size_stats=True
        )
        self.assertCombinerOutputEqual(
            batches, generator, statistics_pb2.FeatureNameStatistics()
        )
    ...
    def test_image_stats_generator_values_threshold_check(self):
        ...
        generator = image_stats_generator.ImageStatsGenerator(
            image_decoder=image_decoder, values_threshold=7, enable_size_stats=True
        )
        self.assertCombinerOutputEqual(batches, generator, expected_result)
        ...
        generator = image_stats_generator.ImageStatsGenerator(
            image_decoder=image_decoder, values_threshold=6, enable_size_stats=True
        )
        self.assertCombinerOutputEqual(batches, generator, expected_result)
...
class ImageStatsGeneratorRealImageTest(test_util.CombinerFeatureStatsGeneratorTest):
    def test_image_stats_generator_real_image(self):
        ...
        generator = image_stats_generator.ImageStatsGenerator(
            is_image_ratio_threshold=0.6, values_threshold=1, enable_size_stats=True
        )
        self.assertCombinerOutputEqual(batches, generator, expected_result)
