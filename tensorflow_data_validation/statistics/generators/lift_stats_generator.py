...
from tensorflow_data_validation.utils import bin_util, schema_util, stats_util
from tensorflow_data_validation.utils.example_weight_map import ExampleWeightMap

_XType = Union[str, bytes]
_YType = Union[str, bytes, int]
...
_SlicedYKey = tfx_namedtuple.TypedNamedTuple(
    "_SlicedYKey", [("slice_key", types.SliceKey), ("y", _YType)]
)
...
_SlicedXKey = tfx_namedtuple.TypedNamedTuple(
    "_SlicedXKey",
    [("slice_key", types.SliceKey), ("x_path", types.FeaturePathTuple), ("x", _XType)],
)
...
_SlicedFeatureKey = tfx_namedtuple.TypedNamedTuple(
    "_SlicedFeatureKey",
    [("slice_key", types.SliceKey), ("x_path", types.FeaturePathTuple)],
)
...
_YRate = tfx_namedtuple.TypedNamedTuple(
    "_YRate", [("y_count", _CountType), ("example_count", _CountType)]
)

_LiftInfo = tfx_namedtuple.TypedNamedTuple(
    "_LiftInfo",
    [
        ("x", _XType),
        ("y", _YType),
...
_LiftValue = tfx_namedtuple.TypedNamedTuple(
    "_LiftValue",
    [("x", _XType), ("lift", float), ("xy_count", _CountType), ("x_count", _CountType)],
)

_LiftSeries = tfx_namedtuple.TypedNamedTuple(
    "_LiftSeries",
    [("y", _YType), ("y_count", _CountType), ("lift_values", Iterable[_LiftValue])],
)
_ValuePresence = tfx_namedtuple.TypedNamedTuple(
    "_ValuePresence",
    [("example_indices", np.ndarray), ("values", np.ndarray), ("weights", np.ndarray)],
)
...
def _get_example_value_presence(
    ...
    assert (
        boundaries is None or not is_binary_like
...
def _to_partial_copresence_counts(
    ...
    num_xy_pairs_batch_copresent: Optional[
        beam.metrics.metric.Metrics.DelegatingDistribution
    ] = None,
    ...
    for x_path in x_paths:
        ...
        x_presence = _get_example_value_presence(
            record_batch, x_path, boundaries=None, weight_column_name=weight_column_name
        )
        ...
        for example_index, x, weight in zip(
            x_presence.example_indices, x_presence.values, x_presence.weights
        ):
...
def _to_partial_counts(
    ...
    slice_key, record_batch = sliced_record_batch
    value_presence = _get_example_value_presence(
...
def _get_unicode_value(value: Union[str, bytes]) -> str:
...
def _make_dataset_feature_stats_proto(
    ...
    for lift_series in sorted(lift_series_list):
        lift_series_proto = cross_stats.categorical_cross_stats.lift.lift_series.add()
        ...
        lift_values_sorted = sorted(
            lift_values_deduped.values(), key=lambda v: (-v.lift, v.x)
        )
...
def _make_placeholder_counts(
    join_result: Tuple[
        types.SliceKey, Tuple[types.FeaturePathTuple, _XType, _CountType], _YType
    ],
...
def _make_conditional_y_rates(
    ...
    conditional_y_rate = _ConditionalYRate(
        x_path=sliced_x_key.x_path, x=sliced_x_key.x, xy_count=xy_count, x_count=x_count
    )
...
def _compute_lifts(
    join_info: Tuple[_SlicedYKey, Dict[str, Iterable[Any]]],
    ...
    for conditional_y_rate in join_inputs["conditional_y_rate"]:
        ...
        yield (
            _SlicedFeatureKey(slice_key, conditional_y_rate.x_path),
            _LiftInfo(
...
class _WeakRefFrozenMapping(collections.abc.Mapping):
...
class _LookupInnerJoinDoFn(beam.DoFn):
    ...
    def __init__(self):
        ...
        self._right_lookup_contruction_seconds_distribution = (
            beam.metrics.Metrics.distribution(
                constants.METRICS_NAMESPACE, "right_lookup_construction_seconds"
            )
        )
    ...
    def process(
        self,
        left_element: Tuple[_JoinKeyType, _LeftJoinValueType],
...
class _FilterLifts(beam.PTransform):
    ...
    def expand(self, lifts: beam.pvalue.PCollection) -> beam.pvalue.PCollection:
        ...
        if self._top_k_per_y:
            # (_LiftSequenceKey(slice, x_path, y, y_count),
            #      [_LiftValue(x, lift, xy_count, x_count)])
            top_k = lifts | "TopK" >> beam.transforms.combiners.Top.PerKey(
                n=self._top_k_per_y, key=top_key
            )
        if self._bottom_k_per_y:
            # (_LiftSequenceKey(slice, x_path, y, y_count),
            #      [_LiftValue(x, lift, xy_count, x_count)])
            bottom_k = lifts | "BottomK" >> beam.transforms.combiners.Top.PerKey(
        ...
        def move_y_info_to_value(
            key: _LiftSeriesKey, lift_values: List[_LiftValue]
        ) -> Tuple[_SlicedFeatureKey, _LiftSeries]:
            return (
                _SlicedFeatureKey(key.slice_key, key.x_path),
                _LiftSeries(y=key.y, y_count=key.y_count, lift_values=lift_values),
            )

        # (_SlicedFeatureKey(slice, x_path),
        #      _LiftSeries(y, y_count, [_LiftValue(x, lift, xy_count, x_count)]))
        return grouped_lifts | "MoveYInfoToValue" >> beam.MapTuple(move_y_info_to_value)
...
class _GetPlaceholderCopresenceCounts(beam.PTransform):
    ...
    def expand(
        self,
        x_counts_and_ys: Tuple[
            beam.PCollection[Tuple[_SlicedXKey, _CountType]],
        ...
        # slice, y
        y_keys_by_slice = y_keys | "MoveYToValue_YKey" >> beam.Map(
            lambda k: (k.slice_key, k.y)
        )
        # slice, (x_path, x, x_count)
        x_counts_by_slice = x_counts | "MoveXToValue_XCountsKey" >> beam.MapTuple(
...
class _GetConditionalYRates(beam.PTransform):
    ...
    def expand(
        self,
        sliced_record_batchs_and_ys: Tuple[
            beam.PCollection[types.SlicedRecordBatch], beam.PCollection[_SlicedYKey]
        ],
        ...
        partial_copresence_counts = (
            ...
            | "ToPartialCopresenceCounts"
            >> beam.FlatMap(
                _to_partial_copresence_counts,
                self._y_path,
                self._x_paths,
                self._y_boundaries,
                self._example_weight_map,
...
class _LiftStatsGenerator(beam.PTransform):
    ...
    def __init__(
        ...
        min_x_count: int,
        top_k_per_y: Optional[int],
        bottom_k_per_y: Optional[int],
        example_weight_map: ExampleWeightMap,
        output_custom_stats: bool,
        name: str,
    ) -> None:
        ...
        if self._schema is not None:
            ...
            if self._y_boundaries is not None:
                ...
                if not y_is_categorical:
                    raise ValueError(
                        "Boundaries must be provided with a non-categorical " "y_path."
                    )
        if x_paths is not None:
            self._x_paths = x_paths
        elif self._schema is not None:
            self._x_paths = set(schema_util.get_categorical_features(schema)) - set(
                [y_path]
            )
        else:
    ...
    def expand(
        ...
        y_rates = sliced_record_batchs | "GetYRates" >> _GetYRates(
            self._y_path, self._y_boundaries, self._example_weight_map.get(self._y_path)
        )
        ...
        return (
            {"conditional_y_rate": conditional_y_rates, "y_rate": y_rates}
            ...
            | "MakeProtos"
            >> beam.Map(
                _make_dataset_feature_stats_proto,
                self._y_path,
                self._y_boundaries,
...
class _UnweightedAndWeightedLiftStatsGenerator(beam.PTransform):
    ...
    def expand(
        ...
        unweighted_protos = (
            sliced_record_batchs | "ComputeUnweightedLift" >> self._unweighted_generator
        )
        ...
        weighted_protos = (
            sliced_record_batchs | "ComputeWeightedLift" >> self._weighted_generator
        )
