...
import abc
from typing import Any, Dict, Generic, Hashable, Iterable, List, Optional, TypeVar
...
class StatsGenerator:
    ...
    def __init__(self, name: str, schema: Optional[schema_pb2.Schema] = None) -> None:
...
class ConstituentStatsGenerator(Generic[CONSTITUENT_ACCTYPE], metaclass=abc.ABCMeta):
    ...
    def merge_accumulators(
        self, accumulators: Iterable[CONSTITUENT_ACCTYPE]
    ) -> CONSTITUENT_ACCTYPE:
        """Merges several accumulators to a single accumulator value.
...
class CompositeStatsGenerator(CombinerStatsGenerator, Generic[CONSTITUENT_ACCTYPE]):
    ...
    def __init__(
        ...
        self._keys, self._constituents = zip(*((c.get_key(), c) for c in constituents))
    ...
    def add_input(
        self, accumulator: List[CONSTITUENT_ACCTYPE], input_record_batch: pa.RecordBatch
    ) -> List[CONSTITUENT_ACCTYPE]:
        batch = input_batch.InputBatch(input_record_batch)
        return [c.add_input(a, batch) for c, a in zip(self._constituents, accumulator)]
