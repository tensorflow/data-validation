...
class WeightedFeatureStatsGeneratorTest(
    parameterized.TestCase, test_util.CombinerStatsGeneratorTest
):
    @parameterized.named_parameters(
        ...
        },
        {
            ...
            "batches": [
                pa.RecordBatch.from_arrays([pa.array([["a"], ["a", "b"]])], ["value"])
            ],
    ...
    def test_single_weighted_feature(
        self,
        batches,
        ...
        generator = weighted_feature_stats_generator.WeightedFeatureStatsGenerator(
            schema
        )
        ...
        expected_stats.custom_stats.add(
            name="min_weight_length_diff", num=expected_min_weight_length_diff
        )
        expected_stats.custom_stats.add(
            name="max_weight_length_diff", num=expected_max_weight_length_diff
        )
    ...
    def test_shared_weight(self):
        ...
        expected_result = {
            types.FeaturePath(["weighted_feature1"]): text_format.Parse(
                """
                path {
            ...
            types.FeaturePath(["weighted_feature2"]): text_format.Parse(
                """
                path {
