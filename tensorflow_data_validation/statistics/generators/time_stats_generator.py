...
_UnixTime = collections.namedtuple("_UnixTime", ["format_constant", "begin", "end"])
...
_UNIX_TIMES = [
    _UnixTime(format_constant=schema_pb2.TimeDomain.UNIX_DAYS, begin=7305, end=21915),
    _UnixTime(
...
def _convert_strptime_to_regex(strptime_str: str) -> str:
...
def _build_all_formats() -> Iterable[str]:
...
class _PartialTimeStats:
    ...
    def update(
        self, values: np.ndarray, value_type: types.FeatureNameStatisticsType
    ) -> None:
        ...
        if value_type == statistics_pb2.FeatureNameStatistics.STRING:
            ...
            for unix_time in _UNIX_TIMES:
                num_matching_values = np.sum(
                    (values >= unix_time.begin) & (values < unix_time.end)
                )
                if num_matching_values > 0:
                    self.matching_formats[unix_time.format_constant] += (
                        num_matching_values
                    )
        else:
            raise ValueError(
...
class TimeStatsGenerator(stats_generator.CombinerFeatureStatsGenerator):
    ...
    def merge_accumulators(
        self, accumulators: Iterable[_PartialTimeStats]
    ) -> _PartialTimeStats:
    ...
    def extract_output(
        self, accumulator: _PartialTimeStats
        ...
        if (
            accumulator.invalidated
            or accumulator.considered < self._values_threshold
            or not accumulator.matching_formats
        ):
            return result

        (most_common_format, most_common_count) = (
            accumulator.matching_formats.most_common(1)[0]
        )
