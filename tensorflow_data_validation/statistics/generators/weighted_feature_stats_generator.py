...
class WeightedFeatureStatsGenerator(stats_generator.CompositeStatsGenerator):
    ...
    def __init__(
        self, schema: schema_pb2.Schema, name: str = "WeightedFeatureStatsGenerator"
    ) -> None:
        constituents = []
        ...
        super(WeightedFeatureStatsGenerator, self).__init__(name, constituents, schema)
    ...
    def extract_composite_output(
        ...
        for weighted_feature in self._schema.weighted_feature:
            ...
            value_count_missing = accumulator[
                count_missing_generator.CountMissingGenerator.key(value, required_paths)
            ]
            feature_result.custom_stats.add(
