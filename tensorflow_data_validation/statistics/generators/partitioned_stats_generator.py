...
def _get_partitioned_statistics_summary(
    statistics: Dict[types.FeaturePath, Dict[str, np.ndarray]],
...
) -> Dict[types.FeaturePath, Dict[str, np.ndarray]]:
...
def _default_assign_to_partition(
    sliced_record_batch: types.SlicedRecordBatch, num_partitions: int
...
def _DefaultPartitionTransform(pcol, num_partitions):  # pylint: disable=invalid-name
    ...
    return pcol | "DefaultPartition" >> beam.Map(
        _default_assign_to_partition, num_partitions
    )
...
class _PartitionedStatisticsAnalyzerAccumulator:
    ...
    def add_input(
        ...
        for feature in statistic.features:
            for stat in feature.custom_stats:
                accumulator.statistics[types.FeaturePath.from_proto(feature.path)][
                    stat.name
                ].append(stat.num)
        return accumulator
...
class _SampleRecordBatchRows(beam.CombineFn):
    ...
    def __init__(self, sample_size: int):
        ...
        self._combine_byte_size = beam.metrics.Metrics.distribution(
            constants.METRICS_NAMESPACE, "sample_record_batch_rows_combine_byte_size"
        )
        # Number of compacts.
    ...
    def add_input(
        ...
        curr_random_ints = np.random.randint(
            0, np.iinfo(np.int64).max, dtype=np.int64, size=(num_rows,)
        )
    ...
    def _compact_impl(
        ...
        if (
            len(accumulator.record_batches) <= 1
...
class _GenerateNonStreamingCustomStats(beam.PTransform):
    ...
    def expand(self, pcoll: beam.pvalue.PCollection) -> beam.pvalue.PCollection:
        ...
        return (
            pcoll
            | "AssignBatchToPartition"
            >> self._stats_fn.partitioner(self._num_partitions)
            | "GroupPartitionsIntoList"
            >> beam.CombinePerKey(
                _SampleRecordBatchRows(self._max_examples_per_partition)
            )
            | "ProcessPartition"
            >> beam.Map(_process_partition, stats_fn=self._stats_fn)
            | "ComputeMetaStats"
            >> beam.CombinePerKey(
                PartitionedStatisticsAnalyzer(
                    min_partitions_stat_presence=self._min_partitions_stat_presence
                )
            )
        )
...
class NonStreamingCustomStatsGenerator(stats_generator.TransformStatsGenerator):
    ...
    def __init__(
        ...
        max_examples_per_partition: int,
        batch_size: int = 1000,
        name: str = "NonStreamingCustomStatsGenerator",
    ) -> None:
