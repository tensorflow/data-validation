...
def _get_flattened_feature_values_without_nulls(feature_array: pa.Array) -> List[Any]:
...
def _get_categorical_feature_encoding(
    category_frequencies: Dict[Any, int], max_encoding_length: int
) -> Dict[Any, int]:
    ...
    for index, value in enumerate(
        sorted(category_frequencies, key=category_frequencies.get, reverse=True)[
...
def _apply_categorical_encoding_to_feature_array(
    feature_array: pa.Array, categorical_encoding: Dict[Any, int], encoding_length: int
) -> List[Any]:
...
def _encode_multivalent_categorical_feature(
    feature_array: pa.Array, max_encoding_length: int
) -> List[int]:
    ...
    return _apply_categorical_encoding_to_feature_array(
        feature_array, categorical_encoding, encoding_length
    )


def _apply_numerical_encoding_to_feature_array(
    feature_array: pa.Array, histogram_bin_boundaries: np.ndarray, encoding_length: int
) -> List[int]:
    ...
    values_indices = np.stack((non_missing_values, non_missing_parent_indices), axis=-1)
...
def _encode_multivalent_numeric_feature(
    ...
    try:
        ...
        logging.exception(
            "Unable to encode examples: %s with error: %s", flattened_feature_values, e
        )
        return None
...
def _encode_univalent_feature(feature_array: pa.Array) -> List[Any]:
    ...
    non_missing_values = np.asarray(flattened)
    nan_mask = pd.isnull(non_missing_values)
    non_nan_pairs = np.stack((non_missing_values, non_missing_parent_indices), axis=-1)[
        ~nan_mask
    ]
...
def _encode_examples(
    ...
    max_encoding_length: int,
) -> Dict[types.FeaturePath, List[Any]]:
    ...
    for feature_name, feature_column in zip(
        examples_record_batch.schema.names, examples_record_batch.columns
    ):
...
class _PartitionFn(beam.DoFn):
    ...
    def __init__(
        self, row_partitions: int, column_partitions: int, label_column: str, seed: int
    ):
...
@beam.typehints.with_output_types(Tuple[Tuple[types.SliceKey, int], pa.RecordBatch])
@beam.ptransform_fn
def _PartitionTransform(
    pcol,
    row_partitions: int,
    column_partitions: int,
...
class MutualInformation(partitioned_stats_generator.PartitionedStatsFn):
    ...
    def _label_feature_is_unique(self, record_batch: pa.RecordBatch):
        for feature_name, feature_array in zip(
            record_batch.schema.names, record_batch.columns
        ):
            feature_path = types.FeaturePath([feature_name])
            if (
                feature_path == self._label_feature
    ...
    def compute(
        ...
        encoded_examples = _encode_examples(
            examples_record_batch,
    ...
    def partitioner(self, num_partitions: int) -> beam.PTransform:
        ...
        return _PartitionTransform(
            num_partitions, self._column_partitions, self._label_feature, self._seed
        )
        # pylint: enable=no-value-for-parameter

    def _normalize_mi_values(self, raw_mi: Dict[types.FeaturePath, Dict[str, float]]):
        ...
        for feature_name, value in raw_mi.items():
            ...
            normalized_mi[feature_name] = {self._custom_stats_key: normalized_value}
    ...
    def _calculate_mi(
        ...
        seed: int,
        k: int = 3,
    ) -> Dict[types.FeaturePath, Dict[str, float]]:
        ...
        for feature_column in examples_dict:
            ...
            if feature_column in self._categorical_features and self._is_unique_array(
                feature_array
            ):
                result[feature_column] = {self._custom_stats_key: 0.0}
                continue
            ...
            feature_categorical_mask = [
                (
                    feature_column in self._categorical_features
                    and feature_column not in self._multivalent_features
                )
