...
class _PartialCrossFeatureStats:
    """Holds partial cross feature statistics for a feature cross."""

    __slots__ = ["sum_x", "sum_y", "sum_square_x", "sum_square_y", "sum_xy", "count"]
...
CrossFeatureStatsGeneratorAccumulator = Dict[
    types.FeatureCross, _PartialCrossFeatureStats
]
...
class CrossFeatureStatsGenerator(stats_generator.CombinerStatsGenerator):
    ...
    def __init__(
        ...
        name: str = "CrossFeatureStatsGenerator",
    ...
    def _get_univalent_values_with_parent_indices(
    ...
    ) -> Dict[types.FeatureName, DataFrame]:
        ...
        for feature_name, feat_arr in zip(examples.schema.names, examples.columns):
            if (
                self._features_needed is not None
            ...
            if not univalent_parent_indices:
                continue
            flattened, value_parent_indices = array_util.flatten_nested(feat_arr, True)
            ...
            df = pd.DataFrame(
                {feature_name: non_missing_values, "parent_index": value_parent_indices}
            )
    ...
    def add_input(
        ...
        # each cross.
        features_for_cross = self._get_univalent_values_with_parent_indices(examples)
            ...
            if skipped_crosses:
                logging.warn(
                    ...
                    ", ".join(
                        sorted(
                            [
                                "_".join([cross[0], cross[1]])
                                for cross in skipped_crosses
                            ]
                        )
                    ),
                )
        for feat_name_x, feat_name_y in feature_crosses:
            ...
            df_x, df_y = (
                features_for_cross[feat_name_x],
            ...
            accumulator[feat_cross].update(
                merged_df[feat_name_x], merged_df[feat_name_y]
            )
        ...
        for feat_cross, cross_feat_stats in accumulator.items():
            ...
            if cross_feat_stats.count > 0:
                ...
                std_dev_x = math.sqrt(
                    max(
                        0,
                        (cross_feat_stats.sum_square_x / cross_feat_stats.count)
                ...
                std_dev_y = math.sqrt(
                    max(
                        0,
                        (cross_feat_stats.sum_square_y / cross_feat_stats.count)
