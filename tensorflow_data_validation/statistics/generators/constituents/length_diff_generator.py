...
class LengthDiffGenerator(stats_generator.ConstituentStatsGenerator):
    ...
    def key(
        ...
        right_path: types.FeaturePath,
        required_paths: Optional[Iterable[types.FeaturePath]] = None,
    ) -> Tuple[Union[str, types.FeaturePath], ...]:
    ...
    def get_key(self) -> Tuple[Union[str, types.FeaturePath], ...]:
        ...
        -------
          The unique key for this set of init args.
        """
        return LengthDiffGenerator.key(
            self._left_path, self._right_path, self._required_paths
        )

    def create_accumulator(self) -> MinMax:
        return float("inf"), float("-inf")

    def add_input(self, accumulator: MinMax, batch: input_batch.InputBatch) -> MinMax:
