...
class _PartialNLStats:
    ...
    def __init__(self, matched: int = 0, considered: int = 0, invalidate=False) -> None:
...
class AverageWordHeuristicNLClassifier(NLClassifierInterface):
    ...
    def __init__(
        ...
        crop_at_length: int = _CROP_AT_LENGTH,
    ) -> None:
    ...
    def classify(self, value: str) -> bool:
        ...
        if (
            self._avg_word_length_min <= avg_word_length <= self._avg_word_length_max
            and len(words) >= self._min_words_per_value
        ):
            return True
        return False


class NLDomainInferringStatsGenerator(stats_generator.CombinerFeatureStatsGenerator):
    ...
    def add_input(
        ...
        def _is_non_utf8(value):
            return isinstance(value, bytes) and stats_util.maybe_get_utf8(value) is None

        is_non_utf_vec = np.vectorize(_is_non_utf8, otypes=[bool])
        classify_vec = np.vectorize(self._classifier.classify, otypes=[bool])
        values = np.asarray(
            array_util.flatten_nested(feature_array)[0].slice(0, _CROP_AT_VALUES)
        )
    ...
    def extract_output(
        ...
        if (
            not accumulator.invalidate
            and accumulator.considered >= self._values_threshold
        ):
