...
def _get_components(
    sparse_features: Iterable[Tuple[types.FeaturePath, schema_pb2.SparseFeature]],
...
class SparseFeatureStatsGenerator(stats_generator.CompositeStatsGenerator):
    ...
    def __init__(
        self, schema: schema_pb2.Schema, name: str = "SparseFeatureStatsGenerator"
    ) -> None:
        ...
        super(SparseFeatureStatsGenerator, self).__init__(name, constituents, schema)

    def extract_composite_output(self, accumulator):
        stats = statistics_pb2.DatasetFeatureStatistics()
        for feature_path, (value, indices) in self._sparse_feature_components.items():
            required_paths = [value] + list(indices)
