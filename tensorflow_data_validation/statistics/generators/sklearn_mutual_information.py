...
from tensorflow_data_validation.utils import schema_util, stats_util
...
def _flatten_and_impute(
    ...
    result = {}
    for column_name, feature_array in zip(examples.schema.names, examples.columns):
        ...
        if pa.types.is_null(feature_array.type):
            ...
            imputed_values_array = np.full(
                shape=num_rows, fill_value=imputation_fill_value
            )
...
class SkLearnMutualInformation(partitioned_stats_generator.PartitionedStatsFn):
    ...
    def __init__(
        self, label_feature: types.FeaturePath, schema: schema_pb2.Schema, seed: int
    ):
    ...
    def compute(
        ...
        flattened_examples = _flatten_and_impute(examples, self._categorical_features)
    ...
    def _convert_categorical_features_to_numeric(self, df: pd.DataFrame) -> List[bool]:
        ...
        for i, column in enumerate(df):
            if column in self._categorical_features:
                ...
                if (
                    unique_elements.size == df[column].shape[0]
                    and not self._label_feature_is_categorical
                ):
    ...
    def _remove_unsupported_feature_columns(
        ...
        for column_name, column in zip(examples.schema.names, examples.columns):
            # only support 1-nested non-struct arrays.
            column_type = column.type
            if (
                arrow_util.get_nest_level(column_type) != 1
...
def _sklearn_calculate_mi_wrapper(
    feature: np.ndarray,
    label: np.ndarray,
    discrete_features: Union[bool, Sequence[bool]],
    seed: int,
    copy: bool,
    is_label_categorical: bool,
) -> Optional[np.ndarray]:
