...
class GetExampleValuePresenceTest(absltest.TestCase):
    ...
    def test_example_value_presence(self):
        ...
        pd.testing.assert_frame_equal(
            ...
            _get_example_value_presence_as_dataframe(
                t, types.FeaturePath(["x"]), boundaries=None, weight_column_name=None
            ),
        )
    ...
    def test_example_value_presence_weighted(self):
        t = pa.RecordBatch.from_arrays(
            [
                pa.array([[1], [1, 1], [1, 2], [2]]),
                pa.array([[0.5], [1.0], [1.5], [2.0]]),
            ],
            ["x", "w"],
        )
        expected_df = pd.DataFrame(
            {"values": [1, 1, 1, 2, 2], "weights": [0.5, 1.0, 1.5, 1.5, 2.0]},
            index=pd.Index([0, 1, 2, 2, 3], name="example_indices"),
        )
        pd.testing.assert_frame_equal(
            expected_df,
            _get_example_value_presence_as_dataframe(
                t, types.FeaturePath(["x"]), boundaries=None, weight_column_name="w"
            ),
        )
...
class ToPartialCountsTest(absltest.TestCase):
    def test_text_to_partial_counts_unweighted(self):
        ...
        for (expected_key, expected_count), (
    ...
    def test_float_to_partial_counts_unweighted(self):
        t = pa.RecordBatch.from_arrays(
            [
                pa.array([[100.0], [200.0], [100.0]]),
            ],
            ["x"],
        )
        x_path = types.FeaturePath(["x"])
        expected_counts = [
            (lift_stats_generator._SlicedXYKey("", x_path, x=100.0, y=None), 2),
            (lift_stats_generator._SlicedXYKey("", x_path, x=200.0, y=None), 1),
        ]
        for (expected_key, expected_count), (
            (actual_key, actual_value),
            actual_count,
...
class ToPartialXCountsTest(absltest.TestCase):
    ...
    def test_to_partial_x_counts_weighted(self):
        ...
        x_path = types.FeaturePath(["x"])
        expected_counts = [
            (lift_stats_generator._SlicedXYKey("", x_path.steps(), x=1, y=None), 2.5),
            (lift_stats_generator._SlicedXYKey("", x_path.steps(), x=2, y=None), 0.5),
        ]
        for (expected_key, expected_count), (actual_key, actual_count) in zip(
            expected_counts,
            lift_stats_generator._to_partial_x_counts(
...
class ToPartialCopresenceCountsTest(absltest.TestCase):
    ...
    def test_to_partial_copresence_counts_weighted(self):
        ...
        x_path = types.FeaturePath(["x"])
        expected_counts = [
            (lift_stats_generator._SlicedXYKey("", x_path.steps(), x=1, y="a"), 0.5),
            (lift_stats_generator._SlicedXYKey("", x_path.steps(), x=1, y="b"), 2.0),
...
class LiftStatsGeneratorTest(test_util.TransformStatsGeneratorTest):
    ...
    def test_lift_string_y_with_boundaries(self):
        ...
        with self.assertRaisesRegex(
            ValueError, r"Boundaries cannot be applied to a " "categorical y_path.*"
        ):
    ...
    def test_lift_int_y_with_no_boundaries(self):
        ...
        with self.assertRaisesRegex(
            ValueError,
            r"Boundaries must be provided with a non-" "categorical y_path.*",
        ):
            lift_stats_generator.LiftStatsGenerator(
    ...
    def test_lift_with_no_schema_or_x_path(self):
        with self.assertRaisesRegex(
            ValueError, r"Either a schema or x_paths must be provided"
        ):
            lift_stats_generator.LiftStatsGenerator(
    ...
    def test_lift_int_y(self):
        ...
        def metrics_verify_fn(metric_results):
            ...
            placeholder_num_right_keys_dists = metric_results.query(
                ...
                .with_name("right_lookup_num_keys")
        ...
        generator = lift_stats_generator.LiftStatsGenerator(
            schema=schema, y_path=types.FeaturePath(["int_y"]), output_custom_stats=True
        )
        self.assertSlicingAwareTransformOutputEqual(
            examples,
            generator,
        ...
        examples = [
            pa.RecordBatch.from_arrays(
                [
                    ...
                    pa.array([["x"], ["x"], ["y"], ["x"]]),
                    pa.array([["cat"], ["dog"], ["cat"], ["dog"]]),
                    pa.array([[0.1], [0.1], [0.5], [0.2]]),
                    pa.array([[0.5], [0.5], [2], [1]]),
                ],
                ["categorical_x1", "categorical_x2", "string_y", "weight_x2", "weight"],
            ),
        ]
        schema = text_format.Parse(
            """
    ...
    def test_lift_y_is_none(self):
        ...
        generator = lift_stats_generator.LiftStatsGenerator(
            schema=schema, y_path=types.FeaturePath(["float_y"]), y_boundaries=[0.5]
        )
        self.assertSlicingAwareTransformOutputEqual(
            examples,
            generator,
    ...
    def test_lift_min_x_count(self):
        ...
        generator = lift_stats_generator.LiftStatsGenerator(
            schema=schema, y_path=types.FeaturePath(["string_y"]), min_x_count=2
        )
        self.assertSlicingAwareTransformOutputEqual(
            examples,
            generator,
    ...
    def test_lift_min_x_count_filters_all(self):
        ...
        expected_result = []
        generator = lift_stats_generator.LiftStatsGenerator(
            schema=schema, y_path=types.FeaturePath(["string_y"]), min_x_count=4
        )
        self.assertSlicingAwareTransformOutputEqual(
            examples,
            generator,
