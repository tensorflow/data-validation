...
from tensorflow_data_validation.statistics.generators import (
    top_k_uniques_sketch_stats_generator as sketch_generator,
)
...
class TopKUniquesSketchStatsGeneratorTest(
    ...
    def test_topk_uniques_sketch_with_single_bytes_feature(self):
        ...
        batches = [
            ...
            pa.RecordBatch.from_arrays(
                [pa.array([["a", "b", "c", "d"]], type=pa.list_(pa.binary()))], ["fa"]
            ),
        ]
    ...
    def test_topk_uniques_combiner_with_weights(self):
        ...
        expected_result = {
            ...
            types.FeaturePath(["fb"]): text_format.Parse(
                """
                  string_stats {
                    unique: 5
                    top_values {
    ...
    def test_topk_uniques_sketch_with_single_unicode_feature(self):
        ...
        batches = [
            pa.RecordBatch.from_arrays(
                [pa.array([["a", "b", "c", "e"], ["a", "c", "d", "a"]])], ["fa"]
            ),
            pa.RecordBatch.from_arrays([pa.array([["a", "b", "c", "d"]])], ["fa"]),
        ]
        expected_result = {
            types.FeaturePath(["fa"]): text_format.Parse(
                """
    ...
    def test_topk_uniques_sketch_zero_row(self):
        batches = [
            pa.RecordBatch.from_arrays(
                [pa.array([], type=pa.list_(pa.binary()))], ["f1"]
            )
        ]
        expected_result = {}
        generator = sketch_generator.TopKUniquesSketchStatsGenerator(
    ...
    @parameterized.named_parameters(
        {"testcase_name": "int", "is_float": False},
        {"testcase_name": "float", "is_float": True},
    )
    def test_topk_uniques_sketch_with_categorical_numeric_feature(self, is_float):
    ...
    def test_topk_struct_leaves(self):
        ...
        expected_result = {
            ...
            types.FeaturePath(["c", "f2"]): text_format.Parse(
                """
              string_stats {
                unique: 4
                top_values {
    ...
    feature {
      ...
      int_domain { is_categorical: true }
        ...
        self.assertCombinerOutputEqual(batches, generator, expected_feature_stats={})
    ...
    def test_invalid_utf8_values(self):
        ...
        batches = [
            pa.RecordBatch.from_arrays(
                [
                    pa.array(
                        [[b"a", b"b", b"\x80", b"a"], [b"a", b"\xc1", b"\x80", b"a"]]
                    ),
                ],
                ["fa"],
            ),
    ...
    @parameterized.named_parameters(
        ...
        },
        {
          ...
          """.replace(
                "__LARGE_BYTES__", "f" * (sketch_generator._LARGE_STRING_THRESHOLD + 1)
            ),
        },
        {
    ...
    def test_string_domain_categorization(self, schema, expected_partial_stats):
        ...
        if expected_partial_stats:
            expected_result = {
                types.FeaturePath(["fa"]): text_format.Parse(
                    expected_partial_stats, statistics_pb2.FeatureNameStatistics()
                )
            }
        generator = sketch_generator.TopKUniquesSketchStatsGenerator(
            num_top_values=2,
