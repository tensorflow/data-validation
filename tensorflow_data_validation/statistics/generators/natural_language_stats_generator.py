...
class _TokenStats:
    ...
    def __iadd__(self, other: "_TokenStats") -> "_TokenStats":
        ...
        for attr, fn in [
            ("per_sequence_min_frequency", min),
...
class _PartialNLStats:
    ...
    def __init__(
        ...
        self.vocab_token_length_quantiles = sketches.QuantilesSketch(
            _QUANTILES_SKETCH_ERROR,
            _QUANTILES_SKETCH_NUM_ELEMENTS,
    ...
    def __iadd__(self, other: "_PartialNLStats") -> "_PartialNLStats":
        ...
        for list_name in [
            "reported_sequences_coverage",
            "reported_sequences_avg_token_length",
        ]:
...
def _update_accumulator_with_in_vocab_string_tokens(
    accumulator: _PartialNLStats, token_list: List[str]
):
...
def _update_accumulator_with_token_statistics(
    accumulator: _PartialNLStats,
    row: List[Union[int, str]],
    tokens: Union[Set[int], Set[str]],
    ...
    for t in tokens:
        ...
        for attr, fn in [
            ...
            if accum_freq is not None:
                setattr(
                    accumulator.token_statistics[t], attr, fn(accum_freq, num_occur)
                )
            else:
                setattr(accumulator.token_statistics[t], attr, num_occur)
        for i in norm_indices:
            accumulator.token_statistics[t].positions[
                int(i * num_histogram_buckets)
            ] += 1


def _update_accumulator_reported_sequences(
    accumulator: _PartialNLStats,
    resolved_entry: List[Union[str, int]],
    oov_string_tokens: Set[str],
):
    """Update reported sequences in accumulator."""
    token_lens = [
        len(i)
        for i in resolved_entry
    ...
    for attr, metric in [
        ("reported_sequences_coverage", coverage),
...
def _update_accumulator_with_sequence_lengths(
    accumulator: _PartialNLStats,
    sequence_length_excluded_int_tokens: Set[int],
    sequence_length_excluded_string_tokens: Set[str],
    max_sequence_length: int,
    int_row: Optional[List[Union[int, str]]],
    string_row: Optional[List[Union[str, int]]],
):
    ...
    if string_row is not None:
        matches = [e for e in string_row if e in sequence_length_excluded_string_tokens]
        sequence_length -= len(matches)
    accumulator.sequence_length_quantiles.AddValues(pa.array([sequence_length]))
    accumulator.min_sequence_length = (
        sequence_length
        if not accumulator.min_sequence_length
        else min(accumulator.min_sequence_length, sequence_length)
    )
    accumulator.max_sequence_length = (
        sequence_length
        if not accumulator.max_sequence_length
        else max(accumulator.max_sequence_length, sequence_length)
    )
...
def _compute_int_statistics(
    row: List[int],
    accumulator: _PartialNLStats,
    excluded_string_tokens: Set[str],
    excluded_int_tokens: Set[int],
    oov_string_tokens: Set[str],
    unused_vocab: Optional[Dict[str, int]],
    rvocab: Optional[Dict[int, str]],
    int_tokens: Set[int],
    string_tokens: Set[str],
    sequence_length_excluded_int_tokens: Set[int],
    sequence_length_excluded_string_tokens: Set[str],
    ...
    if row:
        _update_accumulator_with_token_statistics(
            accumulator, row, int_tokens, num_histogram_buckets
        )
        string_row = None
        if rvocab:
            string_row = [rvocab.get(r, r) for r in row]
            _update_accumulator_with_token_statistics(
                accumulator, string_row, string_tokens, num_histogram_buckets
            )

        _update_accumulator_reported_sequences(
            accumulator, string_row if string_row else row, oov_string_tokens
        )
    ...
    if filtered_entry_str_list:
        _update_accumulator_with_in_vocab_string_tokens(
...
def _compute_str_statistics(
    ...
    unused_rvocab: Optional[Dict[int, str]],
    int_tokens: Set[int],
    string_tokens: Set[str],
    sequence_length_excluded_int_tokens: Set[int],
    sequence_length_excluded_string_tokens: Set[str],
    ...
    if row:
        _update_accumulator_with_token_statistics(
            accumulator, row, string_tokens, num_histogram_buckets
        )
        ...
        if vocab:
            ...
            _update_accumulator_with_token_statistics(
                accumulator, int_row, int_tokens, num_histogram_buckets
            )
        _update_accumulator_with_sequence_lengths(
            accumulator,
            sequence_length_excluded_int_tokens,
    ...
    for entry in row:
        if entry in excluded_string_tokens:
            continue
        if vocab is not None and entry in vocab and vocab[entry] in excluded_int_tokens:
            continue
    ...
    if filtered_entry_list:
        _update_accumulator_with_in_vocab_string_tokens(
            accumulator, filtered_entry_list
        )
...
def _populate_token_statistics(
    ...
    if stats.num_sequences:
        ...
        token_proto.fraction_of_sequences = float(stats.num_sequences) / num_examples
        ...
        _populate_token_position_histogram(token_proto, stats, num_histogram_buckets)
...
class NLStatsGenerator(stats_generator.CombinerFeatureStatsGenerator):
    ...
    def __init__(
        ...
        vocab_paths: Optional[Dict[str, str]],
        num_histogram_buckets: int,
        num_quantiles_histogram_buckets: int,
        num_rank_histogram_buckets: int,
    ) -> None:
    ...
    def merge_accumulators(
        self, accumulators: Iterable[_PartialNLStats]
    ) -> _PartialNLStats:
    ...
    def extract_output(
        ...
        if self._num_rank_histogram_buckets:
            _populate_token_rank_histogram(
                nls, accumulator, self._num_rank_histogram_buckets
            )
        if accumulator.token_statistics:
            for name, stats in accumulator.token_statistics.items():
                _populate_token_statistics(
                    name,
                    self._num_histogram_buckets,
                    accumulator.num_examples,
                    nls.token_statistics.add(),
                    stats,
                )

        for r in (
            accumulator.reported_sequences_coverage
