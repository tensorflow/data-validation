...
from tensorflow_data_validation.utils import (
    example_weight_map as example_weight_map_util,
)
from tensorflow_data_validation.utils import (
    feature_partition_util,
    quantiles_util,
    schema_util,
    stats_util,
...
class _PresenceAndValencyStats:
    ...
    __slots__ = [
        ...
        "num_values_summary",
        "is_top_nested",
        "min_innermost_num_values",
        "max_innermost_num_values",
        "innermost_num_values_summary",
    ]

    def __init__(
        self,
        make_quantiles_sketch_fn: Callable[[], Optional[sketches.QuantilesSketch]],
    ...
    def merge_with(self, other: "_PresenceAndValencyStats") -> None:
        ...
        if self.innermost_num_values_summary is not None:
            self.innermost_num_values_summary.Merge(other.innermost_num_values_summary)
    ...
    def update(
        ...
        if self.is_top_nested:
            num_innermost_values = arrow_util.get_arries_innermost_level_value_counts(
                feature_array
            )
            if num_innermost_values.size:
                ...
                if self.innermost_num_values_summary is not None:
                    num_innermost_values_grouped = pa.array(
...
class _PartialCommonStats:
        ...
        if self.presence_and_valency_stats is None:
            ...
            if this_nest_level != other_nest_level:
                raise ValueError(
                    ...
                    f"feature {feature_path}: {this_nest_level} vs {other_nest_level}"
                )
            for self_stats, other_stats in zip(
                self.presence_and_valency_stats, other.presence_and_valency_stats
            ):
    ...
    def update(
        ...
        if self.type is None:
            ...
            raise TypeError(
                "Cannot determine the type of feature %s. "
        ...
        while array_util.is_list_like(feature_array.type):
            presence_mask = ~np.asarray(
                array_util.GetArrayNullBitmapAsByteArray(feature_array)
            ).view(bool)
            num_values = np.asarray(array_util.ListLengthsFromListArray(feature_array))
            num_values_not_none = num_values[presence_mask]
            self.presence_and_valency_stats[level].update(
                feature_array, presence_mask, num_values, num_values_not_none, weights
            )
...
class _PartialNumericStats:
    ...
    __slots__ = [
        ...
        "finite_max",
        "pos_inf_count",
        "pos_inf_weighted_count",
        "quantiles_summary",
        "has_weights",
        "weighted_quantiles_summary",
        "mean_var_accumulator",
    ...
    def update(
        self, feature_array: pa.Array, weights: Optional[np.ndarray] = None
    ) -> None:
        ...
        if weights is not None:
            ...
            assert self.weighted_mean_var_accumulator is not None
            self.weighted_mean_var_accumulator.update(
                values_no_nan_as_double, flat_weights_no_nan
            )
            if self.weighted_quantiles_summary is not None:
                self.weighted_quantiles_summary.AddValues(
                    pa.array(values_no_nan), pa.array(flat_weights_no_nan)
                )
            self.pos_inf_weighted_count += flat_weights_no_nan[
                np.isposinf(values_no_nan)
            ].sum()


class _PartialStringStats:
    ...
    def update(self, feature_array: pa.Array) -> None:
        ...
        if arrow_util.is_binary_like(flattened_values_array.type):
            ...
            self.total_bytes_length += int(
                array_util.GetBinaryArrayTotalByteSize(flattened_values_array)
            )
            ...
            self.total_bytes_length += np.sum(
                np.vectorize(_len_after_conv, otypes=[np.int32])(
                    np.asarray(flattened_values_array)
                )
            )


class _PartialBytesStats:
    ...
    def update(self, feature_array: pa.Array) -> None:
        ...
        # Iterate through the value array and update the partial stats.'
        flattened_values_array, _ = array_util.flatten_nested(feature_array)
        if pa.types.is_floating(flattened_values_array.type) or pa.types.is_integer(
        ...
        if flattened_values_array:
            num_bytes = array_util.GetElementLengths(flattened_values_array).to_numpy()
...
class _PartialBasicStats:
    ...
    def __init__(
        ...
        self.numeric_stats = _PartialNumericStats(
            has_weights=has_weights, make_quantiles_sketch_fn=make_quantiles_sketch_fn
        )
...
def _make_numeric_stats_proto(
    ...
    if total_num_values == 0:
        ...
        if numeric_stats.num_nan > 0:
            result.histograms.add(
                type=statistics_pb2.Histogram.STANDARD
            ).num_nan = numeric_stats.num_nan
            result.histograms.add(
                type=statistics_pb2.Histogram.QUANTILES
            ).num_nan = numeric_stats.num_nan
        return result

    result.mean = float(numeric_stats.mean_var_accumulator.mean)
    result.std_dev = math.sqrt(max(0, numeric_stats.mean_var_accumulator.variance))
    ...
    if has_weights:
        ...
        weighted_variance = max(0, numeric_stats.weighted_mean_var_accumulator.variance)
...
def _make_string_stats_proto(
    string_stats: _PartialStringStats, total_num_values: int
...
def _make_bytes_stats_proto(
    bytes_stats: _PartialBytesStats, total_num_values: int
...
def _make_num_values_custom_stats_proto(
    ...
    for level, presence_and_valency in zip(
        ...
        if presence_and_valency.num_values_summary is None:
            continue
        num_values_quantiles, num_values_counts = _get_quantiles_counts(
            presence_and_valency.num_values_summary, num_histogram_buckets
        )
        ...
        proto.name = f"level_{level}_value_list_length_quantiles"
        ...
        proto = statistics_pb2.CustomStatistic()
        proto.name = f"level_{level}_value_list_length_standard"
...
def _make_feature_stats_proto(
    ...
    num_quantiles_histogram_buckets: int,
    is_bytes: bool,
    categorical_numeric_types: Mapping[types.FeaturePath, "schema_pb2.FeatureType"],
    has_weights: bool,
    num_examples: int,
    weighted_num_examples: int,
    ...
    if inferred_type is not None:
        ...
        # never infer BYTES.
        if is_bytes and inferred_type == statistics_pb2.FeatureNameStatistics.STRING:
    ...
    common_stats_proto = _make_common_stats_proto(
        ...
        parent_basic_stats.common_stats if parent_basic_stats is not None else None,
    ...
    total_num_values = (
        0
        if basic_stats.common_stats.presence_and_valency_stats is None
        else basic_stats.common_stats.presence_and_valency_stats[-1].total_num_values
    )
    ...
    if (
        result.type == statistics_pb2.FeatureNameStatistics.STRING
        or top_k_uniques_stats_util.output_categorical_numeric(
        ...
        string_stats_proto = _make_string_stats_proto(
            basic_stats.string_stats, total_num_values
        )
    ...
    result.custom_stats.extend(
        _make_num_values_custom_stats_proto(
...
_TFDVMetrics = collections.namedtuple(
    "_TFDVMetrics",
    ["num_non_missing", "min_value_count", "max_value_count", "total_num_values"],
)
...
def _update_tfdv_telemetry(accumulator: "_BasicAcctype") -> None:
    ...
    for basic_stats in accumulator.values():
        ...
        presence_and_valency = (
            ...
            if common_stats.presence_and_valency_stats is None
            else common_stats.presence_and_valency_stats[-1]
        )
        # Update type specific metrics.
        type_metrics = metrics[common_stats.type]
        num_non_missing = (
            type_metrics.num_non_missing + presence_and_valency.num_non_missing
        )
        min_value_count = min(
            type_metrics.min_value_count, presence_and_valency.min_num_values
        )
        max_value_count = max(
            type_metrics.max_value_count, presence_and_valency.max_num_values
        )
        total_num_values = (
            type_metrics.total_num_values + presence_and_valency.total_num_values
        )
        metrics[common_stats.type] = _TFDVMetrics(
            num_non_missing, min_value_count, max_value_count, total_num_values
        )
    ...
    for feature_type in metrics:
        type_str = statistics_pb2.FeatureNameStatistics.Type.Name(feature_type).lower()
        type_metrics = metrics[feature_type]
        counter(constants.METRICS_NAMESPACE, "num_" + type_str + "_feature_values").inc(
            int(type_metrics.num_non_missing)
        )
        if type_metrics.num_non_missing > 0:
            counter(
                constants.METRICS_NAMESPACE, type_str + "_feature_values_min_count"
            ).inc(int(type_metrics.min_value_count))
            counter(
                constants.METRICS_NAMESPACE, type_str + "_feature_values_max_count"
            ).inc(int(type_metrics.max_value_count))
            counter(
                constants.METRICS_NAMESPACE, type_str + "_feature_values_mean_count"
            ).inc(int(type_metrics.total_num_values / type_metrics.num_non_missing))
...
class BasicStatsGenerator(stats_generator.CombinerStatsGenerator):
    ...
    def __init__(
        ...
        self._categorical_numeric_types = (
            schema_util.get_categorical_numeric_feature_types(schema) if schema else {}
        )
        ...
        self._feature_config = feature_config or types.PerFeatureStatsConfig.default()

    def _copy_for_partition_index(
        self, index: int, num_partitions: int
    ) -> stats_generator.StatsGenerator:
        if index < 0 or num_partitions <= 1 or index >= num_partitions:
            raise ValueError(
                "Index or num_partitions out of range: %d, %d" % (index, num_partitions)
            )
    ...
    def add_input(
        self, accumulator: _BasicAcctype, examples: pa.RecordBatch
    ) -> _BasicAcctype:
    ...
    def merge_accumulators(
        self, accumulators: Iterable[_BasicAcctype]
    ) -> _BasicAcctype:
        ...
        for accumulator in it:
            ...
            for feature_path, basic_stats in accumulator.items():
                ...
                if existing_stats is None:
                    ...
                    existing_stats.common_stats.merge_with(
                        feature_path, basic_stats.common_stats
                    )
    ...
    def extract_output(
        self, accumulator: _BasicAcctype
