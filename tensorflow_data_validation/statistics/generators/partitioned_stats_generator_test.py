...
_SAMPLE_PARTITION_TESTS = [
    ...
    {
        "testcase_name": "combine_many_to_one",
        "partitioned_record_batches": [
            (
                1,
                pa.RecordBatch.from_arrays(
...
class AssignToPartitionTest(absltest.TestCase):
    ...
    def test_partitioner(self):
        ...
        record_batches = [
            (constants.DEFAULT_SLICE_KEY, record_batch)
        ...
        for (
            unused_slice_key,
            partition_key,
        ), record_batch in partitioned_record_batches:
...
class SampleRecordBatchRows(parameterized.TestCase):
    ...
    def _partition_matcher(self, expected):
        ...
        def _matcher(actual):
            for expected_tuple, actual_tuple in zip(expected, actual):
                ...
                self.assertTrue(
                    actual_rb.equals(expected_rb),
                    f"Record Batches not equal. "
    ...
    def test_sample_partition_combine(
        self, partitioned_record_batches, expected, sample_size, num_compacts
    ):
        ...
        result = (
            p
            | beam.Create(partitioned_record_batches, reshuffle=False)
            | beam.CombinePerKey(
        ...
        num_compacts_metric = metrics.query(
            beam.metrics.metric.MetricsFilter().with_name(
                "sample_record_batch_rows_num_compacts"
            )
        )["counters"]
    ...
    def test_sample_metrics(self):
        ...
        with beam.Pipeline() as p:
            _ = (
                p
                | beam.Create(partitioned_rbs, reshuffle=False)
                | beam.CombinePerKey(
        ...
        for counter_name in expected_counters:
            ...
            self.assertEqual(
                actual_counter[0].committed, expected_counters[counter_name]
            )
        for counter_name in expected_distributions:
            ...
            self.assertEqual(
                actual_counter[0].committed.count,
