...
def _weighted_unique(
    ...
    gb = df.groupby("value", as_index=False, sort=False)[["count", "weight"]].sum()
...
def _to_topk_tuples(
    ...
    Tuple[
        Tuple[types.SliceKey, types.FeaturePathTuple, Any],
    ...
    for feature_path, feature_array, weights in arrow_util.enumerate_arrays(
        record_batch, example_weight_map=example_weight_map, enumerate_leaves_only=True
    ):
    ...
    def expand(self, pcoll: beam.pvalue.PCollection) -> beam.pvalue.PCollection:
        def _sum_pairwise(
            ...
            if isinstance(iter_of_pairs, list):
                arr = np.array(iter_of_pairs, dtype=[("c", np.int64), ("w", float)])
            else:
                arr = np.fromiter(iter_of_pairs, dtype=[("c", np.int64), ("w", float)])
        ...
        top_k = (
            ...
            | "Unweighted_ToFeatureValueCount"
            >> beam.MapTuple(
                # pylint: disable=g-long-lambda
                lambda k, v: (
                    k,
                    [top_k_uniques_stats_util.FeatureValueCount(t[1], t[0]) for t in v],
                )
        ...
        uniques = (
            ...
            | "Uniques_ConvertToSingleFeatureStats"
            >> beam.MapTuple(
                # pylint: disable=g-long-lambda
                lambda k, v: (
                    k[0],
                    top_k_uniques_stats_util.make_dataset_feature_stats_proto_unique_single(
                        feature_path_tuple=k[1], num_uniques=v
                    ),
                )
                # pylint: enable=g-long-lambda
            )
        ...
        return result_protos | "FlattenTopKUniquesFeatureStatsProtos" >> beam.Flatten()
...
class TopKUniquesStatsGenerator(stats_generator.TransformStatsGenerator):
    ...
    def __init__(
        self,
        name: str = "TopKUniquesStatsGenerator",
