...
from tensorflow_data_validation.statistics.generators import (
    natural_language_stats_generator as nlsg,
)
from tensorflow_data_validation.utils import test_util


class NaturalLanguageStatsGeneratorTest(test_util.CombinerFeatureStatsGeneratorTest):
    def setUp(self):
        ...
        self._int_nlp_feature_with_vocab_and_token_constraints_path = types.FeaturePath(
            ["int_nlp_feature_with_vocab_and_token_constraints"]
        )
    ...
    def test_partial_stats_iadd(self):
        ...
        token_occurrence_counts = stats.token_occurrence_counts.Estimate().to_pylist()
        self.assertListEqual(
            token_occurrence_counts,
            [
                {"values": b"bar", "counts": 3.0},
                {"values": b"baz", "counts": 1.0},
                {"values": b"foo", "counts": 1.0},
            ],
        )
    ...
    def _create_expected_feature_name_statistics(
        ...
        if sorted_token_names_and_counts:
            for index, (token_name, count) in enumerate(sorted_token_names_and_counts):
    ...
    def test_nl_generator_invalidation_check_no_nld(self):
        ...
        accumulator = generator.add_input(
            accumulator, self._non_nlp_feature_path, valid_input
        )
    ...
    def test_nl_generator_invalidation_check_empty_nld(self):
        ...
        accumulator = generator.add_input(
            accumulator, self._int_nlp_feature_empty_domain, valid_input
        )
        self.assertTrue(accumulator.invalidate)
    ...
    def test_nl_generator_string_feature_vocab(self):
        ...
        with tempfile.NamedTemporaryFile() as vocab_file:
            ...
            generator = nlsg.NLStatsGenerator(
                self._schema, {"my_vocab": vocab_file.name}, 0, 0, 0
            )
