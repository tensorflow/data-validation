...
VALID_FORMATS_TESTS = [
    ...
    },
    {
        ...
        "input_batch": pa.array(
            [["2018-11-30T23:59", "2018/11/30 23:59", "Fri Nov 30 10:47:02 2018"]]
        ),
        "expected_matching_formats": {
            "%Y-%m-%dT%H:%M": 1,
            "%Y/%m/%d %H:%M": 1,
...
class TimeStatsGeneratorValidFormatsTest(parameterized.TestCase):
    ...
    def test_time_stats_generator_valid_formats(
        self, input_batch, expected_matching_formats
    ):
        ...
        accumulator = generator.add_input(
            generator.create_accumulator(), types.FeaturePath([""]), input_batch
        )
        self.assertDictEqual(expected_matching_formats, accumulator.matching_formats)
...
class TimeStatsGeneratorTest(test_util.CombinerFeatureStatsGeneratorTest):
    ...
    def test_time_stats_generator_invalidated_exits_add_input_early(self, mock_update):
    ...
    @mock.patch.object(time_stats_generator._PartialTimeStats, "update")
    def test_time_stats_generator_no_values_exits_add_input_early(self, mock_update):
        generator = time_stats_generator.TimeStatsGenerator()
        accumulator = generator.create_accumulator()
    ...
    def test_time_stats_generator_match_ratio_with_same_valid_format(self):
        ...
        input_batches = [
            ...
            pa.array(
                [["not-valid", "not-valid", "not-valid"], ["not-valid", "not-valid"]]
            ),
        ]
    ...
    def test_time_stats_generator_integer_formats(self):
        ...
        self.assertCombinerOutputEqual(
            ...
            statistics_pb2.FeatureNameStatistics(
                custom_stats=[
                    statistics_pb2.CustomStatistic(
                        name="domain_info", str=("time_domain {integer_format: 1}")
                    ),
                    statistics_pb2.CustomStatistic(name="time_match_ratio", num=0.5),
                ]
            ),
