...
class ImageStatsGenerator(stats_generator.CombinerFeatureStatsGenerator):
    ...
    def add_input(
        self,
        accumulator: _PartialImageStats,
    ...
    def merge_accumulators(
        self, accumulators: Iterable[_PartialImageStats]
    ) -> _PartialImageStats:
    ...
    def extract_output(
        self, accumulator: _PartialImageStats
        ...
        if (
            ...
            or accumulator.total_num_values < self._values_threshold
            or (
                1
                - (
                    float(accumulator.counter_by_format[""])
                    / accumulator.total_num_values
                )
            )
            < self._is_image_ratio_threshold
        ):
            return result
