...
from tensorflow_data_validation.statistics import stats_impl, stats_options
from tensorflow_data_validation.statistics.generators import (
    cross_feature_stats_generator,
    stats_generator,
)
from tensorflow_data_validation.utils import slicing_util, test_util
...
_GENERATE_STATS_TESTS = [
    {
        "testcase_name": "feature_allowlist",
        "record_batches": [
            pa.RecordBatch.from_arrays(
                [
                    pa.array([[1.0, 2.0]], type=pa.list_(pa.float32())),
                    pa.array([[b"a", b"b", b"c", b"e"]], type=pa.list_(pa.binary())),
            ...
            pa.RecordBatch.from_arrays(
                [
                    ...
                    pa.array([[b"a", b"c", b"d", b"a"]], type=pa.list_(pa.binary())),
            ...
            pa.RecordBatch.from_arrays(
                [
                    ...
                    pa.array([[b"a", b"b", b"c", b"d"]], type=pa.list_(pa.binary())),
    ...
    {
        "testcase_name": "combiner_feature_stats_generator_on_struct_leaves",
        "record_batches": [
            pa.RecordBatch.from_arrays(
                [
                    pa.array(
            ...
            pa.RecordBatch.from_arrays([pa.array([[{"f1": [{"f2": [4]}]}]])], ["c"]),
        ],
        "options": stats_options.StatsOptions(
    ...
    {
        "testcase_name": "custom_feature_generator",
        "record_batches": [
            pa.RecordBatch.from_arrays(
                [
    ...
    },
    {
        ...
        "record_batches": [
            pa.RecordBatch.from_arrays(
                [
                    pa.array(
                        [[b"This should be natural text"]], type=pa.list_(pa.binary())
                    ),
        ...
        * 90
        + [
            pa.RecordBatch.from_arrays(
                [
                    pa.array(
                        [[b"Thisshouldnotbenaturaltext"]], type=pa.list_(pa.binary())
                    ),
    ...
    {
        "testcase_name": "semantic_domains_disabled",
        "record_batches": [
            pa.RecordBatch.from_arrays(
                [
                    pa.array(
    ...
    {
        "testcase_name": "flat_sparse_feature",
        "record_batches": [
            pa.RecordBatch.from_arrays(
                [pa.array([None]), pa.array([None])], ["value_feature", "index_feature"]
            ),
            pa.RecordBatch.from_arrays(
    ...
    {
        "testcase_name": "struct_leaf_sparse_feature",
        "record_batches": [
            pa.RecordBatch.from_arrays(
                [
                    pa.array(
    ...
    {
        "testcase_name": "cross_feature_stats",
        "record_batches": [
            pa.RecordBatch.from_arrays(
                [
...
_GENERATE_STATS_WITH_FEATURE_PARTITIONS_TESTS = copy.deepcopy(_GENERATE_STATS_TESTS)
...
_GENERATE_STATS_NO_IN_MEMORY_TESTS = [
    {
        ...
        "options": stats_options.StatsOptions(label_feature="string_y"),
        "expected_result_proto_text": """
            datasets {
              num_examples: 4
               cross_features {
...
_SLICING_FN_TESTS = [
    {
        "testcase_name": "feature_value_slicing_slice_fns",
        "record_batches": _SLICE_TEST_RECORD_BATCHES,
        "options": stats_options.StatsOptions(
...
_SLICING_FN_TESTS_SHARDED = [
    {
        "testcase_name": "feature_value_slicing_slice_fns_with_shards",
        "record_batches": _SLICE_TEST_RECORD_BATCHES,
        "options": stats_options.StatsOptions(
            experimental_slice_functions=[
                slicing_util.get_feature_value_slicer({"b": None})
            ...
            num_quantiles_histogram_buckets=2,
            enable_semantic_domain_stats=True,
            experimental_result_partitions=999,
        ),  # 999 >> #features.
        "expected_result_proto_text": _SLICED_STATS_TEST_RESULT,
        "expected_shards": 9,  # 3 slices * 3 shards / slice.
    },
]

_SLICING_FN_TESTS_SHARDED_EMPTY_INPUTS = [
    {
        "testcase_name": "feature_value_slicing_slice_fns_with_shards_empty_inputs",
        "record_batches": _EMPTY_RECORD_BATCHES,
        "options": stats_options.StatsOptions(
            experimental_slice_functions=[
                slicing_util.get_feature_value_slicer({"b": None})
            ],
...
_SLICING_SQL_TESTS = [
    {
        "testcase_name": "feature_value_slicing_slice_sqls",
        "record_batches": [
            pa.RecordBatch.from_arrays(
                [
                    pa.array([[1.0, 2.0]], type=pa.list_(pa.float32())),
...
class StatsImplTest(parameterized.TestCase):
    ...
    def test_stats_impl(
        ...
        expected_result = text_format.Parse(
            expected_result_proto_text, statistics_pb2.DatasetFeatureStatisticsList()
        )
        ...
        with beam.Pipeline() as p:
            result = (
                p
                | beam.Create(record_batches, reshuffle=False)
    ...
    @pytest.mark.xfail(
        run=False, reason="PR 260 This test fails and needs to be fixed."
    )
    def test_stats_impl_slicing_sql(self):
        ...
        expected_result = text_format.Parse(
            _SLICED_STATS_TEST_RESULT, statistics_pb2.DatasetFeatureStatisticsList()
        )
        with beam.Pipeline() as p:
            result = (
                p
    ...
    def test_nld_features(self):
        ...
        expected_result.datasets[0].features[
            0
        ].string_stats.common_stats.num_non_missing = 1
        expected_result.datasets[0].features[
            0
        ].string_stats.common_stats.min_num_values = 1
        expected_result.datasets[0].features[
            0
        ].string_stats.common_stats.max_num_values = 1
        expected_result.datasets[0].features[
            0
        ].string_stats.common_stats.avg_num_values = 1.0
        for _ in range(10):
            expected_result.datasets[0].features[
                0
            ].string_stats.common_stats.num_values_histogram.buckets.add(
                low_value=1.0, high_value=1.0, sample_count=0.1
            )
        expected_result.datasets[0].features[
            0
        ].string_stats.common_stats.num_values_histogram.type = 1
        expected_result.datasets[0].features[
            0
        ].string_stats.common_stats.tot_num_values = 1
        expected_result.datasets[0].features[0].string_stats.rank_histogram.buckets.add(
        ...
        custom_nl_stats = (
            ...
            .features[0]
            .custom_stats.add(name="nl_statistics")
        )
    ...
    def test_generate_statistics_in_memory(
        self, record_batches, options, expected_result_proto_text, schema=None
    ):
        expected_result = text_format.Parse(
            expected_result_proto_text, statistics_pb2.DatasetFeatureStatisticsList()
        )
    ...
    def test_stats_impl_custom_generators(self):
        ...
        class CustomPTransform(beam.PTransform):
            def expand(self, pcoll):
                ...
                return [(None, stats_proto1), (None, stats_proto2)]
    ...
    def test_generate_statistics_in_memory_empty_examples(self):
        ...
        compare.assertProtoEqual(self, result, expected_result, normalize_numbers=True)

    def test_generate_statistics_in_memory_valid_custom_generator(self):
    ...
    def test_generate_statistics_in_memory_invalid_custom_generator(self):
        ...
        with self.assertRaisesRegex(
            TypeError,
            "Statistics generator.* found object of type " "TransformStatsGenerator.",
        ):
    ...
    def test_tfdv_telemetry(self):
        ...
        for counter_name in expected_result:
            ...
            self.assertEqual(actual_counter[0].committed, expected_result[counter_name])
