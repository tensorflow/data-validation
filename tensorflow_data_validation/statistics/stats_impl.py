...
def _increment_counter(counter_name: str, element: int):  # pylint: disable=invalid-name
    counter = beam.metrics.Metrics.counter(constants.METRICS_NAMESPACE, counter_name)
    counter.inc(element)
    return element
...
def _TrackDistinctSliceKeys(  # pylint: disable=invalid-name
    ...
    return (
        ...
        | "IncrementCounter"
        >> beam.Map(lambda x: _increment_counter("num_distinct_slice_keys", x))
    )
...
def _split_generator_types(
    ...
    List[stats_generator.TransformStatsGenerator],
...
class GenerateSlicedStatisticsImpl(beam.PTransform):
        ...
        for idx in range(self._options.experimental_num_feature_partitions):
            ...
            result.append(
                _CombinerStatsGeneratorsCombineFn(
                    index_generators, self._options.desired_batch_size
                )
            )
        return result
    ...
    def expand(self, dataset: beam.PCollection[types.SlicedRecordBatch]):
        ...
        _ = dataset | metrics_util.IncrementJobCounters(generator_name_counts)
        ...
        result_protos = []
        (transform_generators, unpartitioned_combiners, partitioned_combiners) = (
            _split_generator_types(
        ...
        if unpartitioned_combiners:
            combine_fns.append(
                _CombinerStatsGeneratorsCombineFn(
                    unpartitioned_combiners, self._options.desired_batch_size
                )
            )
        ...
        fanout = max(128, 20 * int(math.ceil(math.sqrt(len(combine_fns)))))
        for i, combine_fn in enumerate(combine_fns):
            result_protos.append(
                dataset
                | "RunCombinerStatsGenerators[%d]" % i
                >> beam.CombinePerKey(combine_fn).with_hot_key_fanout(fanout)
            )
        result_protos = result_protos | "FlattenFeatureStatistics" >> beam.Flatten()
        result_protos = (
            result_protos | "AddPlaceholderStatistics" >> _AddPlaceholderStatistics()
        )  # pylint: disable=no-value-for-parameter
        # Combine result_protos into a configured number of partitions.
        return (
            result_protos
            | "AddSliceKeyToStatsProto"
            >> beam.Map(_add_slice_key, self._is_slicing_enabled)
            | "MakeDatasetFeatureStatisticsListProto"
            >> beam.Map(_make_singleton_dataset_feature_statistics_list_proto)
            | "SplitIntoFeaturePartitions"
            >> beam.ParDo(
            ...
            | "MergeStatsProtos"
            >> beam.CombinePerKey(merge_util.merge_dataset_feature_statistics_list)
            | "Values" >> beam.Values()
        )
...
def get_generators(
    ...
    if options.enable_semantic_domain_stats:
        semantic_domain_feature_stats_generators = [
            ...
            natural_language_domain_inferring_stats_generator.NLDomainInferringStatsGenerator(),
...
def _get_default_generators(
    ...
    stats_generators = [
        basic_stats_generator.BasicStatsGenerator(
            ...
            num_quantiles_histogram_buckets=options.num_quantiles_histogram_buckets,
...
def _schema_has_sparse_features(schema: schema_pb2.Schema) -> bool:
    ...
    def _has_sparse_features(feature_container: Iterable[schema_pb2.Feature]) -> bool:
...
class _CombinerStatsGeneratorsCombineFn(beam.CombineFn):
    ...
    def _for_each_generator(
        self, func: Callable[..., Any], *args: Iterable[Any]
    ) -> List[Any]:
        ...
        """
        return [
            func(gen, *args_for_func)
            for gen, args_for_func in zip(self._generators, zip(*args))
        ]

    def _should_do_batch(
        self, accumulator: _CombinerStatsGeneratorsCombineFnAcc, force: bool
    ) -> bool:
        ...
        if accumulator.curr_byte_size >= self._MERGE_RECORD_BATCH_BYTE_SIZE_THRESHOLD:
    ...
    def _maybe_do_batch(
        self, accumulator: _CombinerStatsGeneratorsCombineFnAcc, force: bool = False
    ) -> None:
    ...
    def merge_accumulators(
        self, accumulators: Iterable[_CombinerStatsGeneratorsCombineFnAcc]
    ) -> _CombinerStatsGeneratorsCombineFnAcc:
    ...
    def extract_output(
        ...
        generator_outputs = self._for_each_generator(
            lambda gen, acc: gen.extract_output(acc), accumulator.partial_accumulators
        )
...
def generate_partial_statistics_in_memory(
    ...
    for generator in stats_generators:
        result.append(generator.add_input(generator.create_accumulator(), record_batch))
    return result
...
def generate_statistics_in_memory(
    ...
    partial_stats = generate_partial_statistics_in_memory(
        record_batch, options, stats_generators
    )
...
def extract_statistics_output(
    ...
    outputs = [
        ...
        for (gen, stats) in zip(
            stats_generators, partial_stats
...
class CombinerFeatureStatsWrapperGenerator(stats_generator.CombinerStatsGenerator):
    ...
    def __init__(
        self,
        feature_stats_generators: List[stats_generator.CombinerFeatureStatsGenerator],
        name: str = "CombinerFeatureStatsWrapperGenerator",
    ...
    def add_input(
        self,
        wrapper_accumulator: WrapperAccumulator,
        input_record_batch: pa.RecordBatch,
    ) -> WrapperAccumulator:
        ...
        for feature_path, feature_array, _ in arrow_util.enumerate_arrays(
            input_record_batch, example_weight_map=None, enumerate_leaves_only=True
        ):
    ...
    def merge_accumulators(
        self, wrapper_accumulators: Iterable[WrapperAccumulator]
    ) -> WrapperAccumulator:
    ...
    def compact(self, wrapper_accumulator: WrapperAccumulator) -> WrapperAccumulator:
