...
class StatsOptions:
    ...
    def __repr__(self):
        return "<{}>".format(", ".join(f"{k}={v!r}" for k, v in self.__dict__.items()))
    ...
    @generators.setter
    def generators(
        self, generators: Optional[List[stats_generator.StatsGenerator]]
    ) -> None:
        if generators is not None:
            if not isinstance(generators, list):
                raise TypeError(
                    "generators is of type %s, should be a list."
    ...
    def feature_allowlist(
        ...
        if feature_allowlist is not None and not isinstance(feature_allowlist, list):
            raise TypeError(
                "feature_allowlist is of type %s, should be a list."
    ...
    @vocab_paths.setter
    def vocab_paths(
        self, vocab_paths: Optional[Dict[types.VocabName, types.VocabPath]]
    ) -> None:
        if vocab_paths is not None and not isinstance(vocab_paths, dict):
            raise TypeError(
                "vocab_paths is of type %s, should be a dict."
    ...
    def num_values_histogram_buckets(self, num_values_histogram_buckets: int) -> None:
        ...
        if num_values_histogram_buckets <= 1:
            raise ValueError(
                "Invalid num_values_histogram_buckets %d" % num_values_histogram_buckets
            )
    ...
    def num_histogram_buckets(self, num_histogram_buckets: int) -> None:
        if num_histogram_buckets < 1:
            raise ValueError("Invalid num_histogram_buckets %d" % num_histogram_buckets)
    ...
    def num_quantiles_histogram_buckets(
        self, num_quantiles_histogram_buckets: int
    ) -> None:
        if num_quantiles_histogram_buckets < 1:
            raise ValueError(
                "Invalid num_quantiles_histogram_buckets %d"
    ...
    def desired_batch_size(self, desired_batch_size: Optional[int]) -> None:
        if desired_batch_size is not None and desired_batch_size < 1:
            raise ValueError("Invalid desired_batch_size %d" % desired_batch_size)
    ...
    def semantic_domain_stats_sample_rate(
        ...
        if semantic_domain_stats_sample_rate is not None:
            if not 0 < semantic_domain_stats_sample_rate <= 1:
                raise ValueError(
                    "Invalid semantic_domain_stats_sample_rate %f"
    ...
    def use_sketch_based_topk_uniques(
        ...
        if (
            self.schema
            and not use_sketch_based_topk_uniques
            and schema_pb2.FLOAT
            in schema_util.get_categorical_numeric_feature_types(self.schema).values()
        ):
            raise ValueError(
    ...
    def experimental_result_partitions(self, num_partitions: int) -> None:
        if num_partitions > 0:
            ...
            raise ValueError(
                "Unsupported experimental_result_partitions <= 0: %d" % num_partitions
            )
    ...
    def experimental_num_feature_partitions(self, feature_partitions: int) -> None:
    ...
    def per_feature_stats_config(self) -> types.PerFeatureStatsConfig:
        return self._per_feature_stats_config or types.PerFeatureStatsConfig.default()
