...
from tensorflow_data_validation.utils import feature_partition_util, test_util
...
class FeaturePartitionUtilTest(absltest.TestCase):
    def test_splits_record_batch(self):
        ...
        record_batch = pa.RecordBatch.from_arrays(
            [feature1, feature2, feature3], ["a", "b", "c"]
        )
        ...
        self.assertCountEqual(
            ...
            partitioner.assign.call_args_list,
        )

        # Verify we got the right output slices.
        expected = {
            ("slice_key", 99): pa.RecordBatch.from_arrays(
                [feature1, feature3], ["a", "c"]
            ),
            ("slice_key", 43): pa.RecordBatch.from_arrays([feature2], ["b"]),
        }
        ...
        for key, partitioned_record_batch in partitions:
            ...
            test_util.make_arrow_record_batches_equal_fn(self, [expected_batch])(
    ...
    def test_splits_record_batch_with_universal_features(self):
        ...
        # Verify we got the right output slices.
        expected = {
            ("slice_key", 0): pa.RecordBatch.from_arrays(
                [feature1, feature3], ["a", "c"]
            ),
            ("slice_key", 1): pa.RecordBatch.from_arrays(
                [feature2, feature3], ["b", "c"]
            ),
            ("slice_key", 2): pa.RecordBatch.from_arrays([feature3], ["c"]),
            ("slice_key", 3): pa.RecordBatch.from_arrays([feature3], ["c"]),
        }
        self.assertCountEqual(expected.keys(), [x[0] for x in partitions])
        for key, partitioned_record_batch in partitions:
            expected_batch = expected[key]
...
_KEY_AND_SPLIT_TEST_CASES = [
  ...
  cross_features {
    ...
    {
        "testcase_name": "two_datasets_same_name_same_feature",
        "num_partitions": 9999,
        "statistics": [
            """
        datasets: {
  ...
  features {
    ...
    {
        "testcase_name": "two_datasets_different_name_same_feature",
        "num_partitions": 9999,
        "statistics": [
            """
        datasets: {
  ...
  features {
    ...
    {
        "testcase_name": "does_not_crash_embedded_null_b236190177",
        "num_partitions": 10,
        "statistics": [
            """
        datasets: {
