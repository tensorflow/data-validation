...
def generate_feature_partitions(
    ...
    partition_to_features = collections.defaultdict(lambda: ([], []))  # type: Mapping[int, Any]
    # Arrange output columns normal, universal, with columns within each in their
    # original order.
    for column_name, column in zip(
        sliced_record_batch[1].schema.names, sliced_record_batch[1].columns
    ):
        ...
        for dataset in statistics.datasets:
            for feature in dataset.features:
                if feature.name:
                    ...
                    partition = self._hasher.assign_sequence(
                        dataset.name, *feature.path.step
                    )
            ...
            for cross_feature in dataset.cross_features:
                partition = self._hasher.assign_sequence(
                    dataset.name, *cross_feature.path_x.step, *cross_feature.path_y.step
                )
                dataset_copy = _copy_with_no_features(dataset)
