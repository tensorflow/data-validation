...
class WeightedMeanVarAccumulator:
    ...
    def _combine(
        self, b_count: int, b_mean: float, b_variance: float, b_weights_mean: float
    ):
        ...
        var = a_variance + ratio * (
            b_variance - a_variance + (b_mean - new_mean) * (b_mean - a_mean)
        )
...
class MeanCovAccumulator:
    ...
    def _combine(
        ...
        if new_count == a_count:
            ...
            new_mean = a_mean + ratio * (b_mean - a_mean)
            new_covariance = (
                a_covariance * (a_count - 1)
                + b_covariance * (b_count - 1)
                + (np.outer((a_mean - new_mean), (a_mean - new_mean) * a_count))
                + (np.outer((b_mean - new_mean), (b_mean - new_mean) * b_count))
            ) / (new_count - 1)
        self.count = new_count
        self.mean = new_mean
