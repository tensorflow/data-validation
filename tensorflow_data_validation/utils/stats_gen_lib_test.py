...
from tensorflow_data_validation.utils import stats_gen_lib, test_util
...
class StatsGenTest(parameterized.TestCase):
    ...
    def _make_example(self, feature_name_to_type_values_tuple_map):
        ...
        for feature_name in feature_name_to_type_values_tuple_map:
            (feature_type, feature_values) = feature_name_to_type_values_tuple_map[
                feature_name
            ]
            if feature_type == "bytes":
                result.features.feature[feature_name].bytes_list.value[:] = (
                    feature_values
                )
            elif feature_type == "float":
                result.features.feature[feature_name].float_list.value[:] = (
                    feature_values
                )
            elif feature_type == "int":
                result.features.feature[feature_name].int64_list.value[:] = (
                    feature_values
                )
            else:
    ...
    def _write_tfexamples_to_tfrecords(self, examples, compression_type):
        ...
        with tf.io.TFRecordWriter(data_location, options=compression_type) as writer:
            for example in examples:
                writer.write(example.SerializeToString())
        return data_location

    _BEAM_COMPRESSION_TYPES = [
        {"testcase_name": "no_compression", "compression_type": CompressionTypes.AUTO},
        {
    ...
    def test_stats_gen_with_tfrecords_of_tfexamples(self, compression_type):
        examples = [
            self._make_example(
                {"a": ("float", [1.0, 2.0]), "b": ("bytes", [b"a", b"b", b"c", b"e"])}
            ),
            self._make_example(
                {
            ...
            self._make_example(
                {"a": ("float", [1.0]), "b": ("bytes", [b"a", b"b", b"c", b"d"])}
            ),
        ]
        tf_compression_lookup = {
            CompressionTypes.AUTO: tf.compat.v1.python_io.TFRecordCompressionType.NONE,
            CompressionTypes.GZIP: tf.compat.v1.python_io.TFRecordCompressionType.GZIP,
        }
    ...
    """,
        ...
        result = stats_gen_lib.generate_statistics_from_tfrecord(
            data_location=input_data_path, stats_options=self._default_stats_options
        )
    ...
    def _write_records_to_csv(self, records, tmp_dir, filename, compression_type=""):
  ...
  features {
    ...
    def test_stats_gen_with_csv_no_header_in_file(self, compression_type):
        records, header, expected_result = self._get_csv_test(
            delimiter=",", with_header=False
        )
    ...
    def test_stats_gen_with_csv_tab_delimiter_no_header_in_file(self):
        ...
        input_data_path = self._write_records_to_csv(
            records, self._get_temp_dir(), "input_data.tsv"
        )
    ...
    def test_stats_gen_with_csv_header_in_multiple_files(self):
        records, _, expected_result = self._get_csv_test(
            delimiter=",", with_header=True
        )
  ...
  features {
    ...
    def test_stats_gen_with_header_in_empty_csv_file(self):
        input_data_path = self._write_records_to_csv(
            [], self._get_temp_dir(), "input_data.csv"
        )
    ...
    def test_stats_gen_with_dataframe(self):
        ...
        result = stats_gen_lib.generate_statistics_from_dataframe(
            dataframe=dataframe, stats_options=self._default_stats_options, n_jobs=1
        )
    ...
    def test_stats_gen_with_dataframe_invalid_njobs_zero(self):
        ...
        with self.assertRaisesRegex(ValueError, "Invalid n_jobs parameter.*"):
            _ = stats_gen_lib.generate_statistics_from_dataframe(
                dataframe=dataframe, stats_options=self._default_stats_options, n_jobs=0
            )
