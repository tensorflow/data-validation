...
FeatureValueCount = tfx_namedtuple.namedtuple(
    "FeatureValueCount", ["feature_value", "count"]
)
...
def make_feature_stats_proto_topk_uniques_custom_stats(
    ...
    # Topk rank histogram.
    topk_custom_stats = result.custom_stats.add(name=_TOPK_SKETCH_CUSTOM_STATS_NAME)
    topk_custom_stats.rank_histogram.CopyFrom(topk_stats.string_stats.rank_histogram)
    ...
    # Add the number of uniques to the FeatureNameStatistics proto.
    result.custom_stats.add(name=_UNIQUES_SKETCH_CUSTOM_STATS_NAME, num=num_unique)
    return result
...
def output_categorical_numeric(
    ...
    if (
        ...
        and schema_type == schema_pb2.INT
    ):
        return True
    if (
        feature_type == statistics_pb2.FeatureNameStatistics.FLOAT
