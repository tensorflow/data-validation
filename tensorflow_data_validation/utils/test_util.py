...
def _clear_histograms(
    ...
    for feature in result.features:
        if feature.HasField("num_stats"):
            ...
            has_hist = (
                _clear(feature.num_stats.weighted_numeric_stats, "histograms")
                or has_hist
            )
        ...
        if common_stats is not None:
            has_hist = _clear(common_stats, "feature_list_length_histogram") or has_hist
...
def make_dataset_feature_stats_list_proto_equal_fn(
    ...
    expected_result_merge_fn: Optional[
        Callable[
            [Iterable[statistics_pb2.DatasetFeatureStatisticsList]],
    ...
    def _matcher(actual: Iterable[statistics_pb2.DatasetFeatureStatisticsList]):
        ...
        try:
            ...
            sorted_expected_datasets = sorted(
                expected_result.datasets, key=lambda d: d.name
            )

            for i in range(len(sorted_actual_datasets)):
                assert_dataset_feature_stats_proto_equal(
                    test,
...
def assert_feature_proto_equal(
    test: absltest.TestCase,
    actual: statistics_pb2.FeatureNameStatistics,
    expected: statistics_pb2.FeatureNameStatistics,
) -> None:
...
def assert_dataset_feature_stats_proto_equal(
    ...
    test.assertEqual(
        expected.name,
        actual.name,
        f"Expected name to be {expected.name}, found {actual.name} in "
        f"DatasetFeatureStatistics {actual}",
    )
    test.assertEqual(
...
class CombinerStatsGeneratorTest(absltest.TestCase):
    ...
    def assertCombinerOutputEqual(
        ...
        expected_feature_stats: Dict[
            types.FeaturePath, statistics_pb2.FeatureNameStatistics
        ],
        expected_cross_feature_stats: Optional[
            Dict[types.FeatureCross, statistics_pb2.CrossFeatureStatistics]
        ] = None,
        ...
        def _verify(output):
            ...
            if only_match_expected_feature_stats:
                ...
                self.assertTrue(
                    set(expected_feature_stats.keys()).issubset(features_in_stats)
                )
            else:
                self.assertEqual(  # pylint: disable=g-generic-assert
                    len(output.features),
                    len(expected_feature_stats),
                    f"{output}, {expected_feature_stats}",
                )
            ...
            self.assertEqual(  # pylint: disable=g-generic-assert
                len(result.cross_features),
                len(expected_cross_feature_stats),
                f"{result}, {expected_cross_feature_stats}",
            )
            for actual_cross_feature_stats in result.cross_features:
                cross = (
                    actual_cross_feature_stats.path_x.step[0],
        ...
        result = generator.extract_output(generator.merge_accumulators(accumulators))
        _verify(result)
...
class _DatasetFeatureStatisticsComparatorWrapper:
...
class TransformStatsGeneratorTest(absltest.TestCase):
    ...
    def assertSlicingAwareTransformOutputEqual(
        ...
        metrics_verify_fn: Optional[
            Callable[[beam.metrics.metric.MetricResults], None]
        ] = None,
        ...
        options = beam.options.pipeline_options.PipelineOptions(runtime_type_check=True)
    ...
    def _matcher(actual_record_batches: Iterable[pa.RecordBatch]):
        ...
        for i in range(len(expected_record_batches)):
            ...
            test.assertEqual(
                ...
                f"Expected {expected_record_batch.num_columns} columns, found {actual_record_batch.num_columns} in record_batch {actual_record_batch}",
            )
            for column_name, expected_column in zip(
                ...
                test.assertGreaterEqual(
                    field_index, 0, f"Unable to find column {column_name}"
                )
                actual_column = actual_record_batch.column(field_index)
                test.assertTrue(
                    actual_column.equals(expected_column),
                    f"{column_name}: {actual_column} vs {expected_column}",
                )
