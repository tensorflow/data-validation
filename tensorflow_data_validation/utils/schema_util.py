...
def load_schema_text(input_path: str) -> schema_pb2.Schema:
...
def is_categorical_feature(feature: schema_pb2.Feature):
    ...
    if feature.type == schema_pb2.BYTES:
        ...
        return (
            feature.HasField("int_domain") and feature.int_domain.is_categorical
        ) or feature.WhichOneof("domain_info") in [
            "bool_domain",
            "natural_language_domain",
        ]
    elif feature.type == schema_pb2.FLOAT:
        return feature.HasField("float_domain") and feature.float_domain.is_categorical
    else:
        return False
...
def get_categorical_numeric_feature_types(
    ...
    for feature_path, feature in get_all_leaf_features(schema):
        if (
            feature_path in categorical_numeric_types
            and categorical_numeric_types[feature_path] != feature.type
        ):
            raise ValueError(
                "Schema contains inconsistently typed duplicates for %s" % feature_path
            )
        if feature.type in (
            schema_pb2.INT,
            schema_pb2.FLOAT,
        ) and is_categorical_feature(feature):
...
def get_multivalent_features(schema: schema_pb2.Schema) -> Set[types.FeaturePath]:
    ...
    return {
        ...
        if not (
            (
                feature.shape
                and feature.shape.dim
                and len(feature.shape.dim) == feature.shape.dim[0].size == 1
            )
            or (feature.value_count and feature.value_count.max == 1)
        )
    }


def look_up_feature(
    feature_name: types.FeatureName, container: Iterable[schema_pb2.Feature]
