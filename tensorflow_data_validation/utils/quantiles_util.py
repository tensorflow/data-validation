...
def _get_bin_weights(
    boundaries: np.ndarray, cum_bin_weights: np.ndarray
...
def generate_quantiles_histogram(
    quantiles: np.ndarray, cumulative_counts: np.ndarray
...
def _overlap(
    bucket: statistics_pb2.Histogram.Bucket,
    low_bound: float,
    high_bound: float,
    first_bucket: bool,
    ...
    if low_bound <= bucket.low_value:
        return (
            (high_bound - bucket.low_value) / (bucket.high_value - bucket.low_value),
            False,
            False,
        )
    ...
    if high_bound >= bucket.high_value:
        return (
            (bucket.high_value - low_bound) / (bucket.high_value - bucket.low_value),
            False,
            False,
        )
    ...
    if low_bound > bucket.low_value and high_bound < bucket.high_value:
        return (
            (high_bound - low_bound) / (bucket.high_value - bucket.low_value),
            False,
            False,
        )
    raise ValueError(
        "Unable to compute overlap between (%f, %f) and %s"
...
def generate_equi_width_histogram(
    ...
    if num_neg_inf:
        result.buckets.add(
            low_value=float("-inf"), high_value=float("-inf"), sample_count=num_neg_inf
        )
    ...
    for i in range(new_boundaries.size - 1):
        ...
        for current_index in range(start_index, len(quantiles_hist.buckets)):
            ...
            sample_count += overlap * quantiles_hist.buckets[current_index].sample_count
            current_index += 1
        result.buckets.add(
    ...
    if num_pos_inf:
        result.buckets.add(
            low_value=float("inf"), high_value=float("inf"), sample_count=num_pos_inf
        )
    return result
