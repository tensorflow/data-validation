...
class MeanVarAccumulatorTest(parameterized.TestCase):
    ...
    def test_merges_random_array(
        self, array_size, distribution_mean, distribution_variance, use_weights
    ):
        rng = np.random.default_rng(4444444)
        values = (
            rng.standard_normal(array_size) * np.sqrt(distribution_variance)
            + distribution_mean
        )
        weights = None
        if use_weights:
    ...
    def test_update_random_array(
        ...
        for idx in range(0, values.size, batch_size):
            if weights is None:
                ...
                accumulator.update(
                    values[idx : idx + batch_size], weights[idx : idx + batch_size]
                )
        self.assertLess(
            _rel_err(accumulator.mean, expected_mean), _RELATIVE_ERROR_TOLERANCE
        )
        self.assertLess(
            _rel_err(accumulator.variance, expected_variance), _RELATIVE_ERROR_TOLERANCE
        )
...
class MeanCovAccumulatorTest(parameterized.TestCase):
    @parameterized.named_parameters(
        {
            ...
            "vectors": np.array([[1], [-6], [15]]),
        },
        {
            "testcase_name": "5d x 5",
            "vectors": np.array(
                [
                    [1, 2.4e-9, -3, 43333, 5.1],
                    [-1, 6.99, 8e12, 9, 250],
    ...
    def test_merges_random_array(
        self, array_size, distribution_mean, distribution_variance, num_vectors
    ):
        rng = np.random.default_rng(4444444)
        vectors = []
    ...
    def test_single_observations(self):
        ...
        self.assertListEqual(
            [0, 0, 0, 0, 0, 0, 0, 0, 0], list(accumulator1.covariance.ravel())
        )
        ...
        self.assertListEqual(
            [0, 0, 0, 0, 0, 0, 0, 0, 0], list(accumulator2.covariance.ravel())
        )

        accumulator1.merge(accumulator2)
        self.assertListEqual([4, 5, 6], list(accumulator1.mean))
        self.assertListEqual(
            [9, 9, 9, 9, 9, 9, 9, 9, 9], list(accumulator1.covariance.ravel())
        )
