...
class RanklabMutualInformationTest(parameterized.TestCase):
    ...
    def testOrdinalIndependentFeatures(self):
        ...
        for method in ["smaller_data", "larger_data"]:
            result = _MI([r0], [r1], [False], [False], estimate_method=method, seed=21)
    ...
    def testCorrelatedGaussians(self):
        ...
        result = _MI(
            [f0], [f1], [False], [False], estimate_method="smaller_data", seed=21
        )
        self.assertAlmostEqual(result, expected, places=2)
        result = _MI(
            [f0], [f1], [False], [False], estimate_method="larger_data", seed=21
        )
        self.assertAlmostEqual(result, expected, places=2)

        # Higher dimension.
        rho = 0.9  # fairly strongly dependent features
        [f0, f1, f2, f3], expected = self._MakeCorrelatedFeatures([1, 2, -3, 4], rho)
    ...
    def testAddingIndependentFeature(self):
        ...
        for method in ["smaller_data", "larger_data"]:
            ...
            mi_rws = _MI(
                [r, w], [s], [False] * 2, [False], estimate_method=method, seed=21
            )
            self.assertAlmostEqual(mi_rws, mi_rs, places=2)

    def testMissingValues(self):
        np.random.seed(23)
        fz = np.array([1.0] * 10000)
        fx = np.random.random(10000)
        fa = np.array([1] * 5000 + [2] * 5000, dtype=float)
        fb = np.array([2.3] * 5000 + [None] * 5000)
        fc = np.array([0.0] * 5000 + [10.0] * 5000)

        for method in ["smaller_data", "larger_data"]:
            result = _MI([fz], [fa], [False], [False], seed=23, estimate_method=method)
            self.assertLess(abs(result), 1e-2)

            result = _MI([fc], [fa], [False], [False], seed=23, estimate_method=method)
            self.assertLess(abs(result - 1), 1e-2)

            result = _MI([fb], [fa], [False], [False], seed=23, estimate_method=method)
            self.assertLess(abs(result - 1), 1e-2)

            # Add an independent feature does not affect.
            result = _MI(
                [fc, fx], [fa], [False] * 2, [False], seed=23, estimate_method=method
            )
            self.assertLess(abs(result - 1), 1e-2)

            result = _MI(
                [fb, fx], [fa], [False] * 2, [False], seed=23, estimate_method=method
            )
            self.assertLess(abs(result - 1), 1e-2)
    ...
    def testFilterFeat(self):
        np.random.seed(3)
        fa = np.array(
            ["cat0"] * 2000 + ["cat1"] * 2000 + ["cat2"] * 2000 + ["cat3"] * 2000
        )  # 4 categories
        ...
        for method in ["smaller_data", "larger_data"]:
            ...
            result = _MI(
                [fg], [filter_feat], [False], [False], seed=23, estimate_method=method
            )
    ...
    def testWeightFeat(self):
        ...
        weight_feat = np.array([1] * 2000 + [0.5] * 2000 + [0.25] * 2000 + [0] * 2000)
    ...
    def testOutputEach(self):
        ...
        for method in ["smaller_data", "larger_data"]:
            for categorical0, categorical1 in [
                (True, True),
                (False, True),
                (False, False),
            ]:
                ...
                self.assertAlmostEqual(
                    np.sum(each_mi[fx == 0]) / n, 3.0 / 8, places=None, delta=1e-2
                )

        for method in ["smaller_data", "larger_data"]:
            for categorical0, categorical1, categorical2 in [
                (False, False, True),
                (False, True, True),
            ]:
                result, each_mi = _MI(
    ...
    def testCategorical(self):
        ...
        d = (
            ...
            + np.random.normal(-0.5, 1.0, a.size)
            + np.random.normal(0.0, 0.3, a.size)
        )
        ...
        for method in ["smaller_data", "larger_data"]:
            result = _MI(
                [b], [a], [False], [True], k=6, estimate_method=method, seed=20
            )
            self.assertLess(abs(result), 2e-2)

            result = _MI(
                [c], [a], [False], [True], k=6, estimate_method=method, seed=20
            )
            self.assertAlmostEqual(result, 0.565, delta=1e2)

            result = _MI(
                [d], [a], [False], [True], k=6, estimate_method=method, seed=20
            )
            self.assertLess(abs(result), 1e-2)

            result = _MI(
                [e], [h], [False], [True], k=6, estimate_method=method, seed=20
            )
            self.assertAlmostEqual(result, 4, delta=1e2)

            result = _MI(
                [g], [h], [False], [True], k=6, estimate_method=method, seed=20
            )
            self.assertAlmostEqual(result, 3, delta=1e2)
            ...
            self.assertAlmostEqual(result, 13.15, delta=1e2)
    ...
    def testAdjustedMutualInformation(self):
        ...
        result = mutual_information_util.mutual_information(
            [f0], [label], [True], [True], seed=11
        )
    ...
    def testMergeCategorical(self):
        ...
        self.assertTrue(
            np.array_equal(np.array([b"a:1:alpha", b"b:2:beta", b"c:3:gamma"]), actual)
        )
    ...
    def testUnitVarianceScale_UniformValues(self):
        ...
        actual = mutual_information_util._unit_variance_scale(arr)
        np.testing.assert_equal(actual[np.isnan(actual)], expected[np.isnan(expected)])
        self.assertTrue(
            np.allclose(
                expected[~np.isnan(expected)], actual[~np.isnan(actual)], atol=1e-5
            )
        )
    ...
    def testFeatureToNumpyArray(self):
        ...
        self.assertEqual(
            ...
            [i for i, v in enumerate(expected) if isinstance(v, float) and np.isnan(v)],
        )
        self.assertEqual(
            [v for v in actual if not isinstance(v, float)],
    ...
    def testDiscreteLabelsAppearingExactlyOnce(self):
        ...
        with self.assertRaisesRegex(
            ...
            mutual_information_util._mi_for_arrays(
                [feat0], [], [], [feat1], np.ones_like(feat1)
            )
