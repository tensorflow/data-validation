...
def _get_combined_statistics(
    ...
    if not isinstance(lhs_statistics, statistics_pb2.DatasetFeatureStatisticsList):
        raise TypeError(
    ...
    if rhs_statistics is not None:
        if not isinstance(rhs_statistics, statistics_pb2.DatasetFeatureStatisticsList):
            raise TypeError(
                "rhs_statistics is of type %s, should be a "
                "DatasetFeatureStatisticsList proto." % type(rhs_statistics).__name__
            )
...
def get_natural_language_statistics_dataframes(
    ...
    Dict[str, Dict[Union[int, str], Union[Dict[str, pd.DataFrame], pd.DataFrame]]]
]:
...
def _get_token_statistics(
    token_statistic: List[statistics_pb2.NaturalLanguageStatistics.TokenStatistics],
) -> pd.DataFrame:
    ...
    for token in token_statistic:
        ...
        nlp_stats_dict[_PER_SEQ_MIN_FREQ_KEY].append(token.per_sequence_min_frequency)
        nlp_stats_dict[_PER_SEQ_MAX_FREQ_KEY].append(token.per_sequence_max_frequency)
        nlp_stats_dict[_PER_SEQ_AVG_FREQ_KEY].append(token.per_sequence_avg_frequency)
        nlp_stats_dict[_POSITIONS_KEY].append(_get_histogram_dataframe(token.positions))
