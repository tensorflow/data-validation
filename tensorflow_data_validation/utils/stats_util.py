...
from tensorflow_data_validation.utils import artifacts_io_impl, io_util
...
def maybe_get_utf8(value: bytes) -> Optional[str]:
...
def get_feature_type(dtype: np.dtype) -> Optional[types.FeatureNameStatisticsType]:
...
def get_feature_type_from_arrow_type(
    feature_path: types.FeaturePath, arrow_type: pa.DataType
) -> Optional[types.FeatureNameStatisticsType]:
    ...
    if not array_util.is_list_like(arrow_type):
        raise TypeError(
            ...
            f"(Large)List<primitive|struct> or null, but feature {feature_path} "
            f"was {arrow_type}."
        )
    ...
    raise TypeError(f"Feature {feature_path} has unsupported arrow type: {arrow_type}")


def make_dataset_feature_stats_proto(
    stats_values: Dict[types.FeaturePath, Dict[str, float]],
    ...
    for feature_path in feature_paths:
        feature_stats_proto = _make_feature_stats_proto(
            stats_values[feature_path], feature_path
        )
...
def _make_feature_stats_proto(
    stats_values: Dict[str, float], feature_path: types.FeaturePath
...
def load_stats_text(input_path: str) -> statistics_pb2.DatasetFeatureStatisticsList:
...
def load_stats_binary(input_path: str) -> statistics_pb2.DatasetFeatureStatisticsList:
...
def load_stats_tfrecord(input_path: str) -> statistics_pb2.DatasetFeatureStatisticsList:
...
def get_custom_stats(
    feature_stats: statistics_pb2.FeatureNameStatistics, custom_stats_name: str
) -> Union[float, str, statistics_pb2.Histogram, statistics_pb2.RankHistogram]:
    ...
    if not isinstance(feature_stats, statistics_pb2.FeatureNameStatistics):
        raise TypeError(
            "feature_stats is of type %s, should be a "
            "FeatureNameStatistics proto." % type(feature_stats).__name__
        )
    ...
    raise ValueError(
        "Custom statistics %s not found in the feature statistics." % custom_stats_name
    )
...
def load_statistics(input_path: str) -> statistics_pb2.DatasetFeatureStatisticsList:
...
class DatasetView:
    ...
    def _init_index(self):
        ...
        for j, cross_feature in enumerate(self._statistics.cross_features):
            feature_id = (
                types.FeaturePath.from_proto(cross_feature.path_x),
...
class FeatureView:
    ...
    def custom_statistic(self, name: str) -> Optional[statistics_pb2.CustomStatistic]:
