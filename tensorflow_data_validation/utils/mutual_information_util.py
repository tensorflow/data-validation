...
def mutual_information(
    ...
    _validate_args(
        feature_list0,
        feature_list1,
        is_categorical_list0,
        is_categorical_list1,
        k,
        estimate_method,
        weight_feature,
        filter_feature,
    ...
    final_mi, each = _mi_for_arrays(
        cf_list0, cf_list1, df_list0, df_list1, weights, k, estimate_method, seed
    )
    if output_each:
...
def adjusted_mutual_information(
    ...
    return _adjusted_mi_for_arrays(
        cf_list0, cf_list1, df_list0, df_list1, weights, k, estimate_method, seed
    )
...
def _mi_for_arrays(
    ...
    assert bool(c_arrs0 + d_arrs0) and bool(
        c_arrs1 + d_arrs1
    ), "Both sides are expected to be nonempty."
    ...
    c_arrs0 = [_replace_none_categorical(_unit_variance_scale(f)) for f in c_arrs0]
    c_arrs1 = [_replace_none_categorical(_unit_variance_scale(f)) for f in c_arrs1]
    ...
    if (arr0 is None and arr1 is None) or (df0 is None and df1 is None):
        mi_c01_d01, each_c01_d01 = 0.0, 0.0
    else:
        arr = np.hstack(
            ([] if arr0 is None else [arr0]) + ([] if arr1 is None else [arr1])
        )
        df = _merge_categorical(
            ([] if df0 is None else [df0]) + ([] if df1 is None else [df1])
        )
        mi_c01_d01, each_c01_d01 = _mi_high_dim_cd(arr, df, k, estimate_method, weights)

    if arr0 is None or arr1 is None:
        mi_c0_c1, each_c0_c1 = 0.0, 0.0
    else:
        mi_c0_c1, each_c0_c1 = _mi_high_dim_cc(arr0, arr1, k, estimate_method, weights)

    if df0 is None or df1 is None:
        mi_d0_d1, each_d0_d1 = 0.0, 0.0
    else:
        mi_d0_d1, each_d0_d1 = _mi_high_dim_dd(df0, df1, weights)

    if arr0 is None or df0 is None:
        mi_c0_d0, each_c0_d0 = 0.0, 0.0
    else:
        mi_c0_d0, each_c0_d0 = _mi_high_dim_cd(arr0, df0, k, estimate_method, weights)

    if arr1 is None or df1 is None:
        mi_c1_d1, each_c1_d1 = 0.0, 0.0
    else:
        mi_c1_d1, each_c1_d1 = _mi_high_dim_cd(arr1, df1, k, estimate_method, weights)

    final_mi = max(0.0, mi_c01_d01 + mi_c0_c1 + mi_d0_d1 - mi_c0_d0 - mi_c1_d1)
...
def _adjusted_mi_for_arrays(
    ...
    mi, _ = _mi_for_arrays(
        c_arrs0, c_arrs1, d_arrs0, d_arrs1, weights, k, estimate_method, seed1
    )
...
def _replace_none_categorical(f: np.ndarray) -> np.ndarray:
    ...
    return np.nan_to_num(f, copy=True, nan=2 * np.nanmax(f) - np.nanmin(f) + _NONE_NUM)
...
def _entropy_discrete(
    discrete_f: np.ndarray, weight_f: np.ndarray
) -> Tuple[float, np.ndarray]:
...
def _assert_feature_list(feature_list: List[np.ndarray], list_name: str) -> None:
...
def _validate_args(
    ...
    assert (
        len(set(len(f) for f in feature_list0 + feature_list1)) == 1
    ), "The features have different number of items."

    assert len(is_categorical_list0) == len(
        feature_list0
    ), "is_categorical_list0 is not the same length as feature_list0."
    assert len(is_categorical_list1) == len(
        feature_list1
    ...
    def assert_feature(f, f_name):
        assert (
            f is None or isinstance(f, np.ndarray) and len(f) == len(feature_list0[0])
        ), "%s must be None or a feature with the same item number." % f_name
...
def _feature_list_to_numpy_arrays(
    ...
    is_categorical_list0: List[bool],
    is_categorical_list1: List[bool],
    weight_feature: Optional[np.ndarray],
    filter_feature: Optional[np.ndarray],
) -> Tuple[
    List[np.ndarray], List[np.ndarray], List[np.ndarray], List[np.ndarray], np.ndarray
]:
    ...
    cf_list0 = select_features(feature_list0, is_categorical_list0, lambda a: not a)
    cf_list1 = select_features(feature_list1, is_categorical_list1, lambda a: not a)
...
def _process_high_dim(
    arr: np.ndarray, radius: int, estimate_method: str, weights: np.ndarray
...
def _mi_high_dim_cc(
    ...
    nn = sklearn.neighbors.NearestNeighbors(metric="chebyshev", n_neighbors=k, n_jobs=1)
    nn.fit(arr)
    ...
    if estimate_method == "smaller_data":
        extra = (scipy.special.digamma(k) + scipy.special.digamma(n_samples)) * weights
    elif estimate_method == "larger_data":
        extra = (
            scipy.special.digamma(k) + scipy.special.digamma(n_samples) - 1.0 / k
        ) * weights
    mi += np.mean(extra)
    each = each0 + each1 + extra

    final_mi = max(0.0, mi / math.log(2))
...
def _mi_high_dim_cd(
    ...
    extra = (
        ...
        + scipy.special.digamma(k_all)
        - scipy.special.digamma(label_counts)
    ) * weights
    mi += np.mean(extra)
...
def _mi_high_dim_dd(
    df0: np.ndarray, df1: np.ndarray, weight_f: np.ndarray
) -> Tuple[float, np.ndarray]:
