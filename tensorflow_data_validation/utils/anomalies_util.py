...
def _make_updated_descriptions(
    reasons: List[anomalies_pb2.AnomalyInfo.Reason],
) -> Tuple[str, str]:
    ...
    if len(reasons) == 1 or all(
        reason.type == anomalies_pb2.AnomalyInfo.SCHEMA_NEW_COLUMN for reason in reasons
    ):
        return (reasons[0].description, reasons[0].short_description)
    else:
        return (
            " ".join([reason.description for reason in reasons]),
...
def remove_anomaly_types(
    ...
    types_to_remove: FrozenSet["anomalies_pb2.AnomalyInfo.Type"],
) -> None:
    ...
    for feature_name, anomaly_info in anomalies.anomaly_info.items():
        ...
        if retained_reasons:
            ...
            # are retained.
            (updated_description, updated_short_description) = (
                _make_updated_descriptions(retained_reasons)
            )
...
def get_anomalies_slicer(anomalies: anomalies_pb2.Anomalies) -> types.SliceFunction:
...
def write_anomalies_text(anomalies: anomalies_pb2.Anomalies, output_path: str) -> None:
...
def load_anomalies_text(input_path: str) -> anomalies_pb2.Anomalies:
...
def load_anomalies_binary(input_path: str) -> anomalies_pb2.Anomalies:
