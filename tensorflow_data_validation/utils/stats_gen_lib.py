...
def _generate_partial_statistics_from_df(
    ...
    for col_name, col_type in zip(dataframe.columns, dataframe.dtypes):
        if not table_util.NumpyKindToArrowType(col_type.kind) or (
            ...
            schema.feature.add(
                name=col_name, type=schema_pb2.INT, bool_domain=schema_pb2.BoolDomain()
            )
...
def get_csv_header(
    data_location: str, delimiter: str, compression_type: str = CompressionTypes.AUTO
) -> List[types.FeatureName]:
    ...
    if not matched_files:
        raise ValueError("No file found in the input data location: %s" % data_location)

    # detect compression base on file extension if it is `AUTO`.
    if compression_type == CompressionTypes.AUTO:
        compression_type = CompressionTypes.detect_compression_type(matched_files[0])
    ...
    if compression_type == CompressionTypes.UNCOMPRESSED:
        ...
        raise ValueError(
            "Compression Type: `%s` is not supported for csv files." % compression_type
        )
    ...
    with tf.io.gfile.GFile(file, "rb") as f:
        with io.TextIOWrapper(gzip.GzipFile(fileobj=f), newline="") as t:  # type: ignore
            try:
                ...
                raise ValueError(
                    "Found empty file when reading the header line: %s" % file
                ) from e
