...
from tensorflow_data_validation.arrow import arrow_util, decoded_examples_to_arrow
...
_CONVERSION_TEST_CASES = [
    dict(
        ...
        input_examples=[
            {
                "\U0001f951": np.array([1, 2, 3], dtype=np.int64),
            }
        ],
        expected_output={
        ...
        input_examples=[
            {
                ...
                "double_feature": np.array([1.0], dtype=np.float64),
                "bytes_feature": np.array([b"abc", b"def"], dtype=object),
                "unicode_feature": np.array(["abc", "def"], dtype=object),
            },
            {
                "int64_feature": np.array([4], dtype=np.int64),
                "int32_feature": np.array([4], dtype=np.int32),
                "float_feature": np.array([2.0, 3.0, 4.0], dtype=np.float32),
                "double_feature": np.array([2.0, 3.0, 4.0], dtype=np.float64),
                "bytes_feature": np.array([b"ghi"], dtype=object),
                "unicode_feature": np.array(["ghi"], dtype=object),
            },
        ],
        expected_output={
            "int64_feature": pa.array([[1, 2, 3], [4]], type=pa.list_(pa.int64())),
            "uint64_feature": pa.array([[1, 2, 3], None], type=pa.list_(pa.uint64())),
            "int32_feature": pa.array([[1, 2, 3], [4]], type=pa.list_(pa.int32())),
            "uint32_feature": pa.array([[1, 2, 3], None], type=pa.list_(pa.uint32())),
            "float_feature": pa.array(
                [[1.0], [2.0, 3.0, 4.0]], type=pa.list_(pa.float32())
            ),
            "double_feature": pa.array(
                [[1.0], [2.0, 3.0, 4.0]], type=pa.list_(pa.float64())
            ),
            "bytes_feature": pa.array(
                [[b"abc", b"def"], [b"ghi"]], type=pa.list_(pa.binary())
            ),
            "unicode_feature": pa.array(
                [[b"abc", b"def"], [b"ghi"]], type=pa.list_(pa.string())
            ),
        },
    ),
    dict(
    ...
    dict(
        ...
        input_examples=[
            ...
            {
                "f2": np.array([1.0, 2.0, 3.0], dtype=np.float32),
            },
            {
        ...
        expected_output={
            ...
            "f2": pa.array([None, [1.0, 2.0, 3.0], None, None], pa.list_(pa.float32())),
            "f3": pa.array([None, None, [b"abc", b"def"], None], pa.list_(pa.binary())),
            "f4": pa.array([None, None, None, [8]], pa.list_(pa.int64())),
        },
    ),
    dict(
...
class DecodedExamplesToArrowPyTest(parameterized.TestCase):
    ...
    def test_invalid_input(self, test_input, expected_error, expected_error_regexp):
