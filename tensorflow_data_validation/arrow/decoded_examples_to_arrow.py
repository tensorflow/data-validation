...
def DecodedExamplesToRecordBatch(
    decoded_examples: List[types.LegacyExample],
) -> pa.RecordBatch:
    ...
    for name, array in six.moves.zip(field_names, value_arrays):
        ...
        if not array_util.is_list_like(array.type):
            raise TypeError(
                f"Expected list arrays for field {name} but got {array.type}"
            )
...
def _GetEmptyRecordBatch(num_rows: int) -> pa.RecordBatch:
    ...
    # removing it.
    t = pa.Table.from_arrays([pa.array([None] * num_rows, type=pa.null())], ["dummy"])
