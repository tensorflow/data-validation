...
_FEATURES_TO_ARRAYS = {
    types.FeaturePath(["f1"]): ExpectedArray(pa.array([[1], [2, 3]]), [0, 1], [1, 2]),
    types.FeaturePath(["w"]): ExpectedArray(pa.array([[1], [2]]), [0, 1], [1, 2]),
    ...
    types.FeaturePath(["f2"]): ExpectedArray(
        pa.array([[{"sf1": ["a", "b"]}], [{"sf2": [{"ssf1": [3]}, {"ssf1": [4]}]}]]),
        [0, 1],
        [2, 4],
    ),
    types.FeaturePath(["f3"]): ExpectedArray(
        pa.array(
            [
                {
                    "sf1": [[1, 2], [3]],
    ...
    types.FeaturePath(["f2", "sf2"]): ExpectedArray(
        pa.array([None, [{"ssf1": [3]}, {"ssf1": [4]}]]), [0, 1], [6, 8]
    ),
    types.FeaturePath(["f2", "sf2", "ssf1"]): ExpectedArray(
        pa.array([[3], [4]]), [1, 1], [4, 4]
    ),
    types.FeaturePath(["f3", "sf1"]): ExpectedArray(
        pa.array([[[1, 2], [3]], None]), [0, 1], [1, 2]
    ),
...
def _MakeEnumerateDataWithMissingDataAtLeaves() -> (
    ...
    full_expected_results = {
        types.FeaturePath(["c"]): pa.array([[], [{"f2": [2.0]}, None], [None], []]),
...
def _MakeEnumerateTestDataWithNullTopLevel() -> (
    ...
    missing_expected_results = {
        types.FeaturePath(["c"]): pa.array([[], None, [None], []], type=test_data_type),
        types.FeaturePath(["c", "f2"]): pa.array([None], type=pa.list_(pa.float64())),
    }
    yield (
...
class ArrowUtilTest(parameterized.TestCase):
    ...
    def testGetWeightFeatureNotFound(self):
        with self.assertRaisesRegex(
            ValueError, r'Weight column "w" not present in the input record batch\.'
        ):
            arrow_util.get_weight_feature(
                pa.RecordBatch.from_arrays(
                    [pa.array([[1], [2]]), pa.array([[1], [3]])], ["u", "v"]
                ),
                weight_column="w",
            )
    ...
    def testGetWeightFeatureNullArray(self):
        with self.assertRaisesRegex(
            ValueError, 'Weight column "w" cannot be ' r"null\."
        ):
            arrow_util.get_weight_feature(
                pa.RecordBatch.from_arrays(
                    [pa.array([[1], [2]]), pa.array([None, None])], ["v", "w"]
                ),
                weight_column="w",
            )
    ...
    def testGetWeightFeatureTooManyValues(self):
        with self.assertRaisesRegex(
            ...
            arrow_util.get_weight_feature(
                pa.RecordBatch.from_arrays(
                    [pa.array([[1], [2, 3]]), pa.array([[1], [2, 2]])], ["v", "w"]
                ),
                weight_column="w",
            )
    ...
    def testEnumerateArraysStringWeight(self):
        ...
        with self.assertRaisesRegex(
            ...
            for _ in arrow_util.enumerate_arrays(
                pa.RecordBatch.from_arrays(
                    [pa.array([[1], [2, 3]]), pa.array([["a"], ["b"]])], ["v", "w"]
                ),
    ...
    def testEnumerateArrays(self):
        for leaves_only, has_weights, wrap_flat_struct_in_list in itertools.product(
            ...
            for k, v in six.iteritems(expected_results):
                ...
                self.assertTrue(
                    actual[0].equals(v[0]),
                    f"leaves_only={leaves_only}; has_weights={has_weights}; "
    ...
    @parameterized.named_parameters(
        ...
        {
            "testcase_name": "select_column_f2",
            "col_fn": lambda x: x == "f2",
            "expected_features": [
    ...
    def testEnumerateArraysWithColumnSelectFn(self, col_fn, expected_features):
        actual = list(
            arrow_util.enumerate_arrays(
                _INPUT_RECORD_BATCH, _EXAMPLE_WEIGHT_MAP, True, column_select_fn=col_fn
            )
        )
        expected = list(
        ...
        for (actual_path, actual_col, actual_w), (
            expected_path,
            expected_col,
            expected_w,
    ...
    def testEnumerateMissingPropagatedInFlattenedStruct(self, batch, expected_results):
        actual_results = {}
    ...
    def testGetColumn(self):
        self.assertTrue(
            arrow_util.get_column(_INPUT_RECORD_BATCH, "f1").equals(
                pa.array([[1], [2, 3]])
            )
        )
        self.assertIsNone(
