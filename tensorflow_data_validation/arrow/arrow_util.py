...
def get_weight_feature(
    input_record_batch: pa.RecordBatch, weight_column: str
) -> np.ndarray:
    ...
    if not pa.types.is_floating(flat_weights_type) and not pa.types.is_integer(
        flat_weights_type
    ):
        raise ValueError(
...
def is_binary_like(data_type: pa.DataType) -> bool:
    ...
    return (
        ...
        or pa.types.is_unicode(data_type)
        or pa.types.is_large_unicode(data_type)
    )
