...
_PairOrFeatureSkew = Union[
    feature_skew_results_pb2.SkewPair, Tuple[str, feature_skew_results_pb2.FeatureSkew]
]


def _get_serialized_feature(
    feature: tf.train.Feature, float_round_ndigits: Optional[int]
) -> str:
...
def _compute_skew_for_features(
    base_feature: tf.train.Feature,
    test_feature: tf.train.Feature,
    ...
    if not _empty_or_null(base_feature) and not _empty_or_null(test_feature):
        ...
        if farmhash.fingerprint64(
            _get_serialized_feature(base_feature, float_round_ndigits)
        ) == farmhash.fingerprint64(
            _get_serialized_feature(test_feature, float_round_ndigits)
        ):
...
def _compute_skew_for_examples(
    ...
    for name in feature_names:
        ...
        skew = _compute_skew_for_features(
            base_feature, test_feature, float_round_ndigits, name
        )
...
def _merge_feature_skew_results(
    ...
    result.diff_count = result.base_only + result.test_only + result.mismatch_count
    return result
...
def _empty_or_null(feature: Optional[tf.train.Feature]) -> bool:
    ...
    if feature is None:
        return True
    if (
        len(feature.bytes_list.value)
        + len(feature.int64_list.value)
        + len(feature.float_list.value)
        == 0
    ):
        return True
    return False
...
class _ExtractIdentifiers(beam.DoFn):
    ...
    def process(self, example: tf.train.Example):
        ...
        keyed_example = (
            str(farmhash.fingerprint64("".join(serialized_feature_values))),
            example,
        )
...
def _get_confusion_feature_value(
    ex: tf.train.Example, name: types.FeatureName
) -> Optional[_ConfusionFeatureValue]:
...
def _yield_confusion_pairs(
    ex_base: tf.train.Example, ex_test: tf.train.Example, configs: List[ConfusionConfig]
) -> Iterator[Tuple[_ConfusionFeatureValue, _ConfusionFeatureValue, types.FeatureName]]:
...
def _confusion_count_to_proto(
    values_count: Tuple[
        Tuple[_ConfusionFeatureValue, _ConfusionFeatureValue, types.FeatureName], int
    ],
    ...
    cc = feature_skew_results_pb2.ConfusionCount(feature_name=feature_name, count=count)
    cc.base.bytes_value = base_val
    cc.test.bytes_value = test_val
    return cc


def _make_match_stats_counter(
    base_with_id_count=0,
...
class _MergeMatchStatsFn(beam.CombineFn):
    ...
    def add_input(
        self, mutable_accumulator: np.ndarray, element: np.ndarray
    ) -> np.ndarray:
        mutable_accumulator += element
        return mutable_accumulator

    def merge_accumulators(self, accumulators: Iterable[np.ndarray]) -> np.ndarray:
...
class _ComputeSkew(beam.DoFn):
    ...
    def __init__(
        ...
        confusion_configs: List[ConfusionConfig],
    ) -> None:
        ...
        if base_examples and test_examples:
            for base_example in base_examples:
                for test_example in test_examples:
                    ...
                    if is_skewed:
                        skew_pair = _construct_skew_pair(
                            result, base_example, test_example
                        )
                    ...
                    if self._confusion_configs is not None:
                        for pair in _yield_confusion_pairs(
                            base_example, test_example, self._confusion_configs
                        ):
...
def _extract_compute_skew_result(
    ...
    beam.PCollection[Tuple[str, feature_skew_results_pb2.FeatureSkew]],
    beam.PCollection[feature_skew_results_pb2.SkewPair],
    beam.PCollection[np.ndarray],
    Optional[
        beam.PCollection[Tuple[_ConfusionFeatureValue, _ConfusionFeatureValue, str]]
    ],
]:
    """Extracts results of _ComputeSkew and fixes type hints."""
    # Fix output type hints.
    # TODO(b/211806179): Revert this hack.
    results_skew_results = results[
        SKEW_RESULTS_KEY
    ] | "FixSkewResultsTypeHints" >> beam.Map(lambda x: x).with_output_types(
        Tuple[str, feature_skew_results_pb2.FeatureSkew]
    )
    results_skew_pairs = results[SKEW_PAIRS_KEY] | "FixSkewPairsTypeHints" >> beam.Map(
        lambda x: x
    ).with_output_types(feature_skew_results_pb2.SkewPair)
    results_match_stats = results[
        MATCH_STATS_KEY
    ] | "FixMatchStatsTypeHints" >> beam.Map(lambda x: x).with_output_types(np.ndarray)
    try:
        results_confusion_tuples = results[
            CONFUSION_KEY
...
def _extract_extract_identifiers_result(
    results_base: beam.pvalue.DoOutputsTuple, results_test: beam.pvalue.DoOutputsTuple
) -> Tuple[
    beam.PCollection[Tuple[str, tf.train.Example]],
    beam.PCollection[np.ndarray],
    beam.PCollection[Tuple[str, tf.train.Example]],
    beam.PCollection[np.ndarray],
]:
    """Extracts results of _ExtractIdentifiers and fixes type hints."""
    keyed_base_examples = results_base[
        _KEYED_EXAMPLE_KEY
    ] | "FixKeyedBaseType" >> beam.Map(lambda x: x).with_output_types(
        Tuple[str, tf.train.Example]
    )
    missing_id_base_examples = results_base[
        _MISSING_IDS_KEY
    ] | "BaseMissingCountsToMatchCounter" >> beam.Map(
        lambda x: _make_match_stats_counter(ids_missing_in_base_count=x)
    )

    keyed_test_examples = results_test[
        _KEYED_EXAMPLE_KEY
    ] | "FixKeyedTestType" >> beam.Map(lambda x: x).with_output_types(
        Tuple[str, tf.train.Example]
    )
    missing_id_test_examples = results_test[
        _MISSING_IDS_KEY
    ] | "TestMissingCountsToMatchCounter" >> beam.Map(
        lambda x: _make_match_stats_counter(ids_missing_in_test_count=x)
    )
...
class DetectFeatureSkewImpl(beam.PTransform):
    ...
    def __init__(
        ...
        confusion_configs: Optional[List[ConfusionConfig]] = None,
    ) -> None:
        ...
        if not identifier_features:
            raise ValueError(
                "At least one feature name must be specified in " "identifier_features."
            )
        ...
        self._allow_duplicate_identifiers = allow_duplicate_identifiers
        self._confusion_configs = [] if confusion_configs is None else confusion_configs

    def expand(
        self, pcollections: Tuple[beam.pvalue.PCollection, beam.pvalue.PCollection]
        ...
        keyed_base_examples_result = (
            ...
            >> beam.ParDo(
                _ExtractIdentifiers(
                    self._identifier_features, self._float_round_ndigits
                )
            ).with_outputs(_KEYED_EXAMPLE_KEY, _MISSING_IDS_KEY)
        )
        ...
        (
            ...
            keyed_test_examples,
            missing_id_test_examples,
        ) = _extract_extract_identifiers_result(
        ...
        results = (
            {"base": keyed_base_examples, "test": keyed_test_examples}
        ...
        (
            ...
            results_match_stats,
            results_confusion_tuples,
        ) = _extract_compute_skew_result(results)
        ...
        # Merge match stats.
        match_stats = (
            [results_match_stats, missing_id_test_examples, missing_id_base_examples]
        ...
        skew_pairs = (
            results_skew_pairs
            | "SampleSkewPairs"  # pytype: disable=attribute-error
...
def skew_results_sink(output_path_prefix: str) -> beam.PTransform:
    ...
    return artifacts_io_impl.feature_skew_sink(
        output_path_prefix, feature_skew_results_pb2.FeatureSkew
    )
...
def skew_pair_sink(output_path_prefix: str) -> beam.PTransform:
    ...
    return artifacts_io_impl.feature_skew_sink(
        output_path_prefix, feature_skew_results_pb2.SkewPair
    )
...
def confusion_count_sink(output_path_prefix: str) -> beam.PTransform:
    ...
    return artifacts_io_impl.feature_skew_sink(
        output_path_prefix, feature_skew_results_pb2.ConfusionCount
    )
...
def match_stats_sink(output_path_prefix: str) -> beam.PTransform:
    ...
    return artifacts_io_impl.feature_skew_sink(
        output_path_prefix, feature_skew_results_pb2.MatchStats
    )


def skew_results_iterator(
    input_pattern_prefix,
) -> Iterator[feature_skew_results_pb2.FeatureSkew]:
...
def skew_pair_iterator(
    input_pattern_prefix,
) -> Iterator[feature_skew_results_pb2.SkewPair]:
...
def match_stats_iterator(
    input_pattern_prefix,
) -> Iterator[feature_skew_results_pb2.MatchStats]:
