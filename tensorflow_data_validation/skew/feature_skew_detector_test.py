...
from tensorflow_data_validation.utils import beam_runner_util, test_util
...
def _unpack_results(results_dict):
    ...
    return (
        results_dict[feature_skew_detector.SKEW_RESULTS_KEY],
...
def get_test_input(include_skewed_features, include_close_floats):
    ...
    for i in range(_IDENTIFIER_RANGE):
        for j in range(_IDENTIFIER_RANGE):
            ...
            shared_example.features.feature[_NO_SKEW_FEATURE].int64_list.value.append(1)
        ...
        if include_close_floats:
            base_example.features.feature[_CLOSE_FLOAT_FEATURE].float_list.value.append(
                1.12345
            )
            test_example.features.feature[_CLOSE_FLOAT_FEATURE].float_list.value.append(
                1.12456
            )
        ...
        if include_skewed_features:
            ...
            base_example.features.feature[_BASE_ONLY_FEATURE].int64_list.value.append(0)
            test_example.features.feature[_TEST_ONLY_FEATURE].int64_list.value.append(1)
...
def _make_ex(
    identifier: str, val_skew: str = "", val_noskew: str = ""
) -> tf.train.Example:
    ...
    if val_noskew:
        ex.features.feature["value_noskew"].bytes_list.value.append(val_noskew.encode())
    return ex
...
class FeatureSkewDetectorTest(parameterized.TestCase):
    ...
    def test_detect_feature_skew(self):
        ...
        with beam.Pipeline(runner=beam_runner_util.get_test_runner()) as p:
            ...
            util.assert_that(
                skew_result, test_util.make_skew_result_equal_fn(self, expected_result)
            )
    ...
    def test_detect_no_skew(self):
        ...
        with beam.Pipeline(runner=beam_runner_util.get_test_runner()) as p:
            baseline_examples = p | "Create Baseline" >> beam.Create(baseline_examples)
            test_examples = p | "Create Test" >> beam.Create(test_examples)
            ...
            util.assert_that(
                skew_sample, make_sample_equal_fn(self, 0, []), "CheckSkewSample"
            )
    ...
    def test_obtain_skew_sample(self):
        ...
        with beam.Pipeline(runner=beam_runner_util.get_test_runner()) as p:
            ...
            util.assert_that(
                skew_sample, make_sample_equal_fn(self, sample_size, potential_samples)
            )
    ...
    def test_duplicate_identifiers_not_allowed_with_duplicates(self):
        ...
        with beam.Pipeline(runner=beam_runner_util.get_test_runner()) as p:
            ...
            util.assert_that(skew_result, test_util.make_skew_result_equal_fn(self, []))

        runner = p.run()
        runner.wait_until_finish()
        result_metrics = runner.metrics()
        actual_counter = result_metrics.query(
            beam.metrics.metric.MetricsFilter().with_name("examplediff_skip_dupe_id")
        )["counters"]
    ...
    def test_telemetry(self):
        ...
        _ = (baseline_data, test_data) | feature_skew_detector.DetectFeatureSkewImpl(
            [_IDENTIFIER1, _IDENTIFIER2]
        )
        runner = p.run()
        runner.wait_until_finish()
        ...
        actual_counter = result_metrics.query(
            beam.metrics.metric.MetricsFilter().with_name(
                "examples_with_missing_identifier_features"
            )
        )["counters"]
        self.assertLen(actual_counter, 1)
        self.assertEqual(actual_counter[0].committed, 1)
    ...
    def test_confusion_analysis(self):
        ...
        def _confusion_result(
            base: str, test: str, feature_name: str, count: int
    ...
    @parameterized.named_parameters(
        {
            ...
            "input_example": text_format.Parse(
                """
        features {
          feature {
            key: "id"
        ...
        """,
            ...
            "expected_error_regex": "int64 features unsupported for confusion analysis",
        },
        {
            "testcase_name": "float_feature",
            "input_example": text_format.Parse(
                """
        features {
          feature {
        ...
        """,
            ...
            "expected_error_regex": "float features unsupported for confusion analysis",
        },
        {
            "testcase_name": "multivalent_feature",
            "input_example": text_format.Parse(
                """
        features {
          feature {
        ...
        """,
            ...
            "expected_error_regex": "multivalent features unsupported for confusion analysis",
        },
    )
    ...
    def test_match_stats(self):
        ...
        with beam.Pipeline(runner=beam_runner_util.get_test_runner()) as p:
            ...
            match_stats = (
                (baseline_examples, test_examples)
                | feature_skew_detector.DetectFeatureSkewImpl(["id"], [])
            )[feature_skew_detector.MATCH_STATS_KEY]
