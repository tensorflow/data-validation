...
from tensorflow_data_validation.pywrap.tensorflow_data_validation_extension import (
    validation as pywrap_tensorflow_data_validation,
)
...
from tensorflow_data_validation.utils import anomalies_util, slicing_util, stats_util
...
def _merge_descriptions(
    ...
    other_anomaly_info: Optional[anomalies_pb2.AnomalyInfo],
) -> str:
    """Merges anomaly descriptions."""
    descriptions = []
    if other_anomaly_info is not None:
        for reason in itertools.chain(anomaly_info.reason, other_anomaly_info.reason):
...
def validate_statistics_internal(
    ...
    if serving_statistics is not None:
        ...
        serving_dataset_statistics = _get_default_dataset_statistics(serving_statistics)

    if previous_version_statistics is not None:
        if not isinstance(
            previous_version_statistics, statistics_pb2.DatasetFeatureStatisticsList
        ):
            raise TypeError(
                "previous_version_statistics is of type %s, should be "
                "a DatasetFeatureStatisticsList proto."
...
def _remove_features_missing_common_stats(
    ...
    for feature in stats.features:
        if (
            feature.HasField("num_stats")
            or feature.HasField("string_stats")
            or feature.HasField("bytes_stats")
            or feature.HasField("struct_stats")
        ):
...
def validate_instance(
    ...
    feature_statistics_list = stats_impl.generate_statistics_in_memory(
        instance, options
    )
    anomalies = validate_statistics(
        feature_statistics_list, options.schema, environment
    )
...
def _detect_anomalies_in_example(
...
def _get_default_dataset_statistics(
    ...
    for dataset in statistics.datasets:
        if dataset.name == constants.DEFAULT_SLICE_KEY:
            logging.warning(
                "Multiple datasets found in statistics. Using the "
    ...
    # error.
    raise ValueError(
        "Only statistics proto with one dataset or the default "
...
class IdentifyAnomalousExamples(beam.PTransform):
    ...
    def expand(
        ...
        return (
            dataset
            | "DetectAnomaliesInExamples"
            >> beam.Map(_detect_anomalies_in_example, options=self.options)
            | "GenerateAnomalyReasonKeys"
            >> beam.ParDo(_GenerateAnomalyReasonSliceKeys())
        )
...
class WriteFeatureSkewResultsToTFRecord(beam.PTransform):
    ...
    def expand(self, feature_skew_results: beam.PCollection) -> beam.pvalue.PDone:
        return (
            ...
            | "WriteFeatureSkewResults"
            >> beam.io.WriteToTFRecord(
                self._output_path,
                shard_name_template="",
                coder=beam.coders.ProtoCoder(feature_skew_results_pb2.FeatureSkew),
            )
        )
...
class WriteSkewPairsToTFRecord(beam.PTransform):
    ...
    def expand(self, skew_pairs: beam.PCollection) -> beam.pvalue.PDone:
        return skew_pairs | "WriteSkewPairs" >> beam.io.WriteToTFRecord(
            self._output_path,
            shard_name_template="",
            coder=beam.coders.ProtoCoder(feature_skew_results_pb2.SkewPair),
        )


def _prepend_slice_path(slice_name: str, path: types.FeaturePath) -> types.FeaturePath:
    steps = path.steps()
    ...
    for dataset in statistics.datasets:
        ...
        for feature in dataset.features:
            ...
            copied_feature.path.CopyFrom(
                _prepend_slice_path(
                    dataset.name, types.FeaturePath.from_proto(copied_feature.path)
                ).to_proto()
            )
        for cross_feature in dataset.cross_features:
            ...
            copied_cross_feature.path_x.CopyFrom(
                _prepend_slice_path(
                    dataset.name,
                    types.FeaturePath.from_proto(copied_cross_feature.path_x),
                ).to_proto()
            )
            copied_cross_feature.path_y.CopyFrom(
                _prepend_slice_path(
                    dataset.name,
                    types.FeaturePath.from_proto(copied_cross_feature.path_y),
                ).to_proto()
            )
...
def _replicate_schema_for_sliced_validation(
    schema: schema_pb2.Schema, slice_names: Set[str]
) -> schema_pb2.Schema:
